...
- `App-Gesamtersparnis` ist der rohe Jackery-App-Wert und kann wie PV-Ertrag aussehen. `Berechnete Ersparnis` ist der lokale Wert aus der netzseitigen AC-Ausgabe des SolarVault nach Umrichter-/Akkueffekten, netzseitigem Eingang, optionaler CT-Netzeinspeisung, Hausverbrauch und konfiguriertem Strompreis.
...
- `App-Gesamtersparnis` bleibt der rohe Cloud-Wert. `Berechnete Ersparnis` wird wenn möglich aus selbst genutzter AC-Energie berechnet, nicht aus PV-Ertrag in Euro. Aktiviere die Detail-Sensoren zur Ersparnis-Berechnung, um Zwischenwerte, Batteriesystem-Verlustenergie, Wechselrichter-/Umwandlungsverlustenergie und die berechnete aktuelle Wechselrichter-/Umwandlungsverlustleistung als Entitäten zu sehen. Details stehen in [`APP_CLOUD_VALUES.md`](APP_CLOUD_VALUES.md).
