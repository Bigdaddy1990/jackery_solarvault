...
The integration is imported directly; a broken import fails the test run
...
async def test_emit_payload_debug_with_callable_forwards_factory_as_is() -> None:
    """Callable factories must be forwarded to the callback as-is, not invoked.

    The coordinator-side callback is responsible for invoking the factory only
    when the dedicated ``payload_debug`` logger is at DEBUG level; _emit_payload_debug
...
def test_rsa_pkcs1v15_encrypt_produces_different_ciphertext_each_call() -> None:
    """RSA PKCS#1 v1.5 is probabilistic — two encryptions of the same plaintext.
...
def test_handle_connect_failure_sets_connected_event_and_marks_not_connected() -> None:
    """After _handle_connect_failure, both is_connected and the event must be in.
