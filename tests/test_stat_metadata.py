...
def test_total_revenue_uses_total_increasing_state_class() -> None:
    """total_revenue must use state_class=TOTAL_INCREASING.

    Source of truth: CHANGELOG "Three-part fix" / Midnight race condition.
    total_revenue is a lifetime cumulative monetary counter the cloud
    reports as monotonically growing. TOTAL_INCREASING lets the Recorder
    detect cloud-side resets and ignore them, instead of misreading the
    brief post-midnight transient as a real loss (the bug that the old
    state_class=TOTAL pairing produced).

    The HA "MONETARY device_class only allows TOTAL or None" validation
    does NOT apply here: the description sets no device_class=MONETARY, it
    only publishes the currency unit. So TOTAL_INCREASING is valid.
    Lock it down so it can never regress back to TOTAL.
    """
    sensor_source = SENSOR_PATH.read_text(encoding="utf-8")
    pattern = re.compile(
    ...
    assert match.group(1) == "TOTAL_INCREASING", (
        f"total_revenue uses state_class={match.group(1)!r}; "
        "must be TOTAL_INCREASING per CHANGELOG Three-part fix / Midnight "
        "race condition (no MONETARY device_class is set on this entity)"
    )
    # Guard the premise: TOTAL_INCREASING is only valid because the entity
    # has no device_class=MONETARY. If one is ever added, this test must be
    # revisited (MONETARY would then require TOTAL or None).
    ...
    assert desc_block is not None
    assert "device_class=" not in desc_block.group(0), (
        "total_revenue must not declare a device_class while using "
        "TOTAL_INCREASING; MONETARY would forbid TOTAL_INCREASING"
    )
