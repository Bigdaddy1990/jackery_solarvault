...
def test_is_on_switch_state_none_falls_back_to_sys_switch() -> None:
    ...
    # FIELD_SWITCH_STATE is present but None; raw = None → falls back to FIELD_SYS_SWITCH
    # Actually the code does: raw = plug.get(FIELD_SWITCH_STATE); if raw is None: raw = plug.get(FIELD_SYS_SWITCH)
...
def test_is_on_switch_state_zero_does_not_fallback_to_sys_switch() -> None:
    """When FIELD_SWITCH_STATE is 0 (falsy but not None), must use it, not FIELD_SYS_SWITCH."""
    sensor = _make_plug_sensor(
        smart_plugs=[{"sn": "SN001", FIELD_SWITCH_STATE: 0, FIELD_SYS_SWITCH: 1}]
    )
    # FIELD_SWITCH_STATE = 0 is not None, so it is used directly (not 1 from FIELD_SYS_SWITCH)
    assert sensor.is_on is False
