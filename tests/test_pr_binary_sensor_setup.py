...
- JackerySmartPlugStateBinarySensor is_on: FIELD_SWITCH_STATE None falls back to FIELD_SYS_SWITCH
"""  # noqa: E501
...
async def test_async_setup_entry_creates_binary_sensors_for_each_device() -> None:
    ...
    assert len(entities) == len(BINARY_DESCRIPTIONS)
...
async def test_async_setup_entry_no_data_no_call() -> None:
    """async_setup_entry must not call async_add_entities when coordinator.data is empty."""  # noqa: E501
...
async def test_async_setup_entry_none_data_no_call() -> None:
    """async_setup_entry must not call async_add_entities when coordinator.data is None."""  # noqa: E501
...
async def test_async_setup_entry_creates_smart_plug_entities() -> None:
    """async_setup_entry must create JackerySmartPlugStateBinarySensor for plugs with serials."""  # noqa: E501
    dev_id = "dev_001"
    plugs = [
        {"sn": "SN-A"},
        {"sn": "SN-B"},
...
async def test_async_setup_entry_skips_plugs_without_serial() -> None:
    """async_setup_entry must skip smart plugs that have no extractable serial number."""  # noqa: E501
    dev_id = "dev_001"
    plugs = [
...
async def test_async_setup_entry_same_signature_no_double_add() -> None:
    """Re-invoking _add_new_entities with unchanged coordinator data must not add entities again."""  # noqa: E501
    dev_id = "dev_001"
    coordinator = _make_coordinator(data={dev_id: _make_device_payload()})
    entry = _make_entry(coordinator)
...
def test_is_on_switch_state_none_falls_back_to_sys_switch() -> None:
    ...
    # FIELD_SWITCH_STATE is present but None; raw = None → falls back to FIELD_SYS_SWITCH  # noqa: E501
    # Actually the code does: raw = plug.get(FIELD_SWITCH_STATE); if raw is None: raw = plug.get(FIELD_SYS_SWITCH)  # noqa: E501
...
def test_is_on_switch_state_zero_does_not_fallback_to_sys_switch() -> None:
    """When FIELD_SWITCH_STATE is 0 (falsy but not None), must use it, not FIELD_SYS_SWITCH."""  # noqa: E501
    sensor = _make_plug_sensor(
        smart_plugs=[{"sn": "SN001", FIELD_SWITCH_STATE: 0, FIELD_SYS_SWITCH: 1}]
    )
    # FIELD_SWITCH_STATE = 0 is not None, so it is used directly (not 1 from FIELD_SYS_SWITCH)  # noqa: E501
    assert sensor.is_on is False
...
def test_extra_state_attributes_plug_index_one() -> None:
    ...
    assert sensor.extra_state_attributes["plug_index"] == 1
...
def test_extra_state_attributes_plug_index_five() -> None:
    ...
    assert sensor.extra_state_attributes["plug_index"] == 5  # noqa: PLR2004
...
def test_extra_state_attributes_switch_state_included_when_none() -> None:
    """extra_state_attributes includes FIELD_SWITCH_STATE even when its value is None."""  # noqa: E501
    ...
    assert attrs[FIELD_SWITCH_STATE] is None
...
async def test_async_setup_entry_deduplicates_same_unique_ids() -> None:
    """Duplicate entity unique_ids across rebuild calls must not produce duplicate entities."""  # noqa: E501
    dev_id = "dev_dup"
    coordinator = _make_coordinator(data={dev_id: _make_device_payload()})
    entry = _make_entry(coordinator)
