...
def test_app_energy_unit_scale_missing_unit() -> None:
    ...
    source = {}
    assert app_energy_unit_scale(source) == 1.0  # ruff: ignore[float-equality-comparison]
...
def test_app_energy_unit_scale_wh() -> None:
    ...
    assert app_energy_unit_scale(source) == 0.001  # ruff: ignore[magic-value-comparison, float-equality-comparison]
