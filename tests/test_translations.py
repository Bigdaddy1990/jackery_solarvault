...
def _leaf_paths(value: Any, prefix: str = "") -> set[str]:  # noqa: ANN401, RUF105
...
def test_service_actions_use_translation_files() -> None:
    ...
    # services.yaml is the source of truth for the registered service set;
    # strings.json and icons.json must cover exactly that set so hassfest
    # and the UI never see an untranslated or icon-less action.
    expected_services = set(
        yaml.safe_load(services_yaml)  # top-level keys are the service ids
    )
