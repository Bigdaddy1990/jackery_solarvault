...
async def test_ct_stat_response_contains_y1_y2_arrays() -> None:
    """CT stat response parsing preserves y1 (primary CT) and y2 (secondary CT) arrays."""  # ruff: ignore[line-too-long]
    api = _api()
    payload = {
        "x": ["00:00", "00:05", "00:10"],
...
async def test_ct_stat_wrapper_delegates_with_system_id() -> None:
    """async_get_device_ct_stat delegates to _async_get_device_period_stat with system_id."""  # ruff: ignore[line-too-long]
    api = _api()
    ...
    assert kwargs["device_id"] == 11  # ruff: ignore[magic-value-comparison]
    assert kwargs["query"].system_id == 3  # ruff: ignore[magic-value-comparison]
    assert kwargs["query"].date_type == DATE_TYPE_DAY
