...
from custom_components.jackery_solarvault.client.api import (
    ...
    JackeryApi,
    JackeryApiError,
    _AuthFailureRecovery,  # ruff: ignore[import-private-name]
...
class TestJackeryApi:
    ...
    def _create_client(self) -> JackeryApi:  # ruff: ignore[no-self-use]
...
class TestMQTTCredentials:
    ...
    def _create_client_with_session(self) -> JackeryApi:  # ruff: ignore[no-self-use]
...
class TestAuthAndRelogin:
    ...
    def test_is_token_expired_response(self) -> None:
        ...
        client = self._create_client()
        # Token expired code
        assert client.is_token_expired_response(200, {"code": 10402}) is True
        # Token expired message
        assert client.is_token_expired_response(200, {"msg": "token expires"}) is True
        assert client.is_token_expired_response(200, {"msg": "token expired"}) is True
        # Not expired
        assert client.is_token_expired_response(200, {"code": 0}) is False
        assert client.is_token_expired_response(200, {"msg": "ok"}) is False
        # Non-dict
...
class TestAuthFailurePaths:
    ...
    async def test_property_retry_survives_auth_refresh_timeout(self) -> None:
        ...
        async def auth_then_timeout() -> tuple[int, object]:
            nonlocal attempts
            attempts += 1
            await asyncio.sleep(0)
            if attempts == 1:
...
class TestAdditionalEndpoints:  # ruff: ignore[too-many-public-methods]
    ...
    async def test_async_sync_offline_statistics(self) -> None:
        ...
        client = self._create_client()
        client._post_json = AsyncMock(return_value={"code": 0, "data": True})  # noqa: E501, RUF100, RUF105, SLF001

        result = await client.async_sync_offline_statistics({"offline": "data"})

        assert result is True
        client._post_json.assert_awaited_once()  # ruff: ignore[private-member-access]
        # pyrefly: ignore [missing-attribute]
        assert client._post_json.await_args.args[1] == {"offline": "data"}  # ruff: ignore[private-member-access]
