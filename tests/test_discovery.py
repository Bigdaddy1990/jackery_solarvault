...
def test_property_device_candidate_parses_cloud_marker_strings() -> None:
    ...
    assert JackerySolarVaultCoordinator._is_property_device_candidate({  # ruff: ignore[private-member-access]
        FIELD_BIND_KEY: 1,
        FIELD_DEV_TYPE: 3,
        FIELD_IS_CLOUD: "false",
        FIELD_MODEL_CODE: 3002,
    })
    assert not JackerySolarVaultCoordinator._is_property_device_candidate({  # ruff: ignore[private-member-access]
        FIELD_BIND_KEY: 1,
        FIELD_DEV_TYPE: 3,
        FIELD_IS_CLOUD: "true",
        FIELD_MODEL_CODE: 3002,
