...
# ---------------------------------------------------------------------------
# from __future__ import annotations — compatibility regression
# ---------------------------------------------------------------------------


def test_ble_module_can_be_imported_with_future_annotations() -> None:
    """The ble module (which now has ``from __future__ import annotations``) must
    import cleanly — i.e. adding PEP 563 deferred evaluation must not break any
    module-level code that relies on immediate annotation evaluation.
    """
    import importlib

    import custom_components.jackery_solarvault.client.ble as ble_mod

    # Re-importing should not raise; the module is already cached after the
    # first import at the top of this file.
    reloaded = importlib.import_module(
        "custom_components.jackery_solarvault.client.ble"
    )
    assert reloaded is ble_mod


def test_aes_encrypt_decrypt_roundtrip_with_aes128_key() -> None:
    """AES-128 (16-byte key) encrypt/decrypt must round-trip correctly.
    ...
    plaintext = b'{"batSoc":87,"acInWatt":230}'
...
def test_aes_encrypt_decrypt_roundtrip_with_aes256_key() -> None:
    """AES-256 (32-byte key) encrypt/decrypt must round-trip correctly.
