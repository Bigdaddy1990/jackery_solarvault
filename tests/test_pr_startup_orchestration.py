...
class TestAsyncFinishEntryStartupDiscovery:
    ...
    async def test_discovery_update_failed_with_cached_snapshot_sets_data(  # noqa: PLR6301
        ...
        from homeassistant.helpers.update_coordinator import UpdateFailed  # noqa: I001, PLC0415
...
class TestAsyncFinishEntryStartupGatherResults:
    ...
    async def test_refresh_failed_with_cached_snapshot_sets_data(self) -> None:  # noqa: PLR6301
        ...
        coordinator.cached_discovery_snapshot = MagicMock(return_value=cached)
        # Production performs the first HTTP refresh via ``async_refresh`` (the
        # result is captured with ``return_exceptions=True``), so the transient
        # failure must be injected there — not on ``async_config_entry_first_refresh``.
        coordinator.async_refresh = AsyncMock(side_effect=UpdateFailed("HTTP 503"))
...
def test_api_module_uses_relative_const_import() -> None:
    ...
    """
    from custom_components.jackery_solarvault.client.api import encrypt_mqtt_body  # noqa: I001, PLC0415
...
def test_client_init_getattr_uses_import_module() -> None:
    ...
    from custom_components.jackery_solarvault.client.mqtt.mqtt_push import (  # noqa: PLC0415
