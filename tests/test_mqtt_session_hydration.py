...
class MockCoordinator:
    ...
    def __init__(self, api: JackeryApi, entry: MockConfigEntry, hass: MockHass):
        self.api = api
        self.entry = entry
        self.hass = hass
        self.data = {}
    ...
    def cached_discovery_snapshot(self) -> dict | None:
        return None

    async def async_load_cached_discovery(self, _label: str) -> bool:
        return False

    async def async_load_local_daily_snapshots(self) -> None:
        pass

    def mark_mqtt_session_cache_loaded(self, persisted: Any) -> bool:
        self._persisted_mqtt_session = persisted
        self._mqtt_session_cache_loaded = True
        return True


async def test_entry_bootstrap_mqtt_session_extracts_valid_snapshot() -> None:
...
async def test_entry_bootstrap_mqtt_session_returns_none_for_missing() -> None:
...
async def test_entry_bootstrap_mqtt_session_returns_none_for_invalid() -> None:
...
async def test_normalize_mqtt_session_snapshot_validates_expiry() -> None:
...
async def test_normalize_mqtt_session_snapshot_accepts_valid() -> None:
...
async def test_mqtt_session_cache_load_save_roundtrip() -> None:
...
async def test_coordinator_api_hydrated_before_layer5_start() -> None:
...
async def test_api_derives_credentials_after_hydration() -> None:
...
async def test_api_returns_none_credentials_without_session() -> None:
...
async def test_mqtt_fingerprint_changes_after_new_login() -> None:
