...
def _temporary_directory() -> Iterator[str]:
    ...
    path = _TEMP_ROOT / f"dotenv-{uuid.uuid4().hex}"
    path.mkdir()
    try:
        yield str(path)
    finally:
...
class TestAsyncDiscoverWithCacheFallback:
    ...
    async def test_blocks_layer5_on_update_failed_with_no_cache(self) -> None:  # noqa: PLR6301
        """Startup may continue (returns True) when HTTP discovery fails with no cache.

        Per the production contract, only an authentication failure stops startup
        (returns False). An UpdateFailed leaves the entry able to continue; Layer 5
        remains blocked because no live HTTP discovery data was applied.
        """
        coordinator = MagicMock()
    ...
    async def test_uses_cache_but_blocks_layer5_on_update_failed(self) -> None:  # noqa: PLR6301
        """Loads cached discovery data and allows startup to continue (returns True).

        The cached snapshot is applied so entities keep their last state, but the
        return value stays True because only an authentication failure stops startup.
        Layer 5 remains blocked because the data came from cache, not live HTTP.
        """
        coordinator = MagicMock()
