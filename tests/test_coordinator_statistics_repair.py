...
def test_gate_hierarchy_passes_clean_snapshot_through() -> None:
    ...
    gated = JackerySolarVaultCoordinator._gate_snapshot_period_hierarchy(
        snapshot,
        today=date(2026, 7, 9),
    )
...
def test_gate_hierarchy_empty_snapshot_is_empty() -> None:
    ...
    assert (
        JackerySolarVaultCoordinator._gate_snapshot_period_hierarchy(
            {},
            today=date(2026, 7, 9),
        )
        == {}
...
async def test_current_import_job_gates_snapshot_before_every_import() -> None:
    ...
    coordinator._async_http_backfill_recent_day_statistics = backfill
    coordinator._async_import_day_chart_statistics = day_import
    coordinator._async_import_app_chart_statistics = period_import
    snapshot = {
        _DEV: {
            "device_pv_stat_year": {"y": [1.0, 2.0]},
            "device_pv_stat_month": {"y": [50.0]},
            "properties": {"batSoc": 50},
        },
    }

    await coordinator._async_import_current_app_chart_statistics_job(snapshot)
...
async def test_startup_sync_stays_pending_without_backfill_progress() -> None:
    ...
    coordinator._async_http_backfill_recent_day_statistics = AsyncMock(
        return_value={"pending_sources": 1},
    )
    coordinator._async_import_day_chart_statistics = AsyncMock(return_value=set())
    coordinator._async_import_app_chart_statistics = AsyncMock(return_value=set())

    await coordinator._async_import_current_app_chart_statistics_job(
...
async def test_repair_missing_statistics_default_options_plans_all_periods() -> None:
    ...
    coordinator._collect_repair_buckets = collect
