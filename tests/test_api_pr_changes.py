...
# ---------------------------------------------------------------------------
# Day-curve null-bucket coalescing in the stored period-stat response
# ---------------------------------------------------------------------------


async def test_day_stat_stored_y_series_coalesces_null_buckets_to_zero() -> None:
    """Stored day-curve y-series must replace null/unparseable buckets with 0.0.

    The stored diagnostics response coalesces ``null``/unparseable buckets to
    0.0 (matching the parsed-path default for missing buckets). The original
    payload returned to the coordinator is left untouched (see
    ``test_day_stat_returned_payload_retains_nulls_live_path_untouched``).
    Negative on-grid/battery values, the x labels, scalar totals and request
    meta must be preserved unchanged.
    """
    api = JackeryApi.__new__(JackeryApi)
    api.last_device_period_stat_responses = {}
    ...
    await api.async_get_device_pv_stat(
        "dev1",
        "sys1",
        date_type=DATE_TYPE_DAY,
        begin_date="2026-05-27",
    ...
    stored = api.last_device_period_stat_responses[
        f"{DEVICE_PV_STAT_PATH}:dev1:{DATE_TYPE_DAY}"
    ]
    section = stored[FIELD_DATA]

    # Null buckets coalesced to 0.0; magnitude and negatives preserved.
    assert section["y"] == [0.0, "1.5", 0.0]
    assert section["y1"] == [0.0, -169, 0.0]
    assert None not in section["y"]
    assert None not in section["y1"]

    # Labels, scalar totals and request meta are untouched.
    assert section["x"] == ["00:00", "00:05", "00:10"]
    assert section["totalSolarEnergy"] == "12.3"
    assert section["unit"] == "kWh"
    assert APP_REQUEST_META in stored


async def test_week_stat_stored_y_series_keeps_null_buckets() -> None:
    """Non-day periods must NOT be coalesced — proves the date_type scope gate.

    Only ``date_type == DATE_TYPE_DAY`` carries chart curves through the
    coalescing helper; week/month/year responses must pass through unchanged.
    """
    api = JackeryApi.__new__(JackeryApi)
    api.last_device_period_stat_responses = {}
    ...
    stored = api.last_device_period_stat_responses[f"{DEVICE_PV_STAT_PATH}:dev1:week"]
    # Week curves are left untouched: the null buckets remain present.
    assert stored[FIELD_DATA]["y"] == [None, "2.0", None]


async def test_day_stat_null_payload_is_not_coalesced() -> None:
    """A top-level null data payload must remain None (no crash, no coercion).

    Regression guard for the existing null-payload diagnostics behavior: the
    ``isinstance`` guard short-circuits when ``data["data"]`` is None.
    """
    api = JackeryApi.__new__(JackeryApi)
    api.last_device_period_stat_responses = {}
...
async def test_day_stat_returned_payload_retains_nulls_live_path_untouched() -> None:
    """Coalescing must touch only the stored copy, never the live payload.

    The payload returned to the coordinator (consumed by the day-curve recorder
    import) must keep its original ``null`` buckets; coalescing them in place
    would advance ``day_power_energy_points`` ``last_bucket_minute`` and emit
    extra trailing zero-fill statistic buckets for partial-data days.
    """
    api = JackeryApi.__new__(JackeryApi)
    api.last_device_period_stat_responses = {}
    ...
    async def _get_json(path: str, params: dict[str, Any]) -> dict[str, Any]:  # noqa: RUF029
        return {
            FIELD_CODE: 0,
            FIELD_DATA: {"y": [1.0, None, None], "unit": "kWh"},
        }
    ...
    # Live payload (coordinator / day-curve input) keeps its nulls untouched.
    assert payload["y"] == [1.0, None, None]

    # Stored diagnostics copy coalesces, and is a distinct object from the live payload.
    stored = api.last_device_period_stat_responses[
        f"{DEVICE_PV_STAT_PATH}:dev1:{DATE_TYPE_DAY}"
    ]
    assert stored[FIELD_DATA]["y"] == [1.0, 0.0, 0.0]
    assert stored[FIELD_DATA] is not payload
