...
def _make_coordinator_stub() -> JackerySolarVaultCoordinator:
    ...
    coordinator._last_http_cycle_completed_monotonic = float("-inf")  # ruff: ignore[private-member-access]
    ...
    coordinator._slow_metrics_interval_sec = 300  # ruff: ignore[private-member-access]
    coordinator._configured_update_interval = MagicMock()  # ruff: ignore[private-member-access]
    ...
    cast("Any", coordinator).entry = SimpleNamespace(entry_id="test_entry")
...
async def test_poll_watchdog_stall_threshold() -> None:  # ruff: ignore[unused-async]
    ...
    coordinator._last_http_cycle_completed_monotonic = time.monotonic() - 100.0  # ruff: ignore[private-member-access]
    coordinator._last_poll_watchdog_request_monotonic = 0.0  # ruff: ignore[private-member-access]

    # Should trigger
    age = 100.0
    threshold = max(4 * 15.0, 60.0)
    assert age > threshold
    assert threshold == 60.0  # ruff: ignore[magic-value-comparison, float-equality-comparison]


async def test_poll_watchdog_stays_silent_when_healthy() -> None:  # ruff: ignore[unused-async]
...
async def test_merge_concurrent_updates_prioritizes_http_properties() -> None:  # ruff: ignore[unused-async]
    ...
    merged = coordinator._merge_concurrent_coordinator_updates(baseline, result)  # ruff: ignore[private-member-access]
...
async def test_merge_concurrent_updates_preserves_layer5_deltas() -> None:  # ruff: ignore[unused-async]
    ...
    # Use real merge function - it should preserve Layer-5 deltas for non-property fields  # ruff: ignore[line-too-long]
    def mock_merge(device_id, current, incoming):  # ruff: ignore[missing-type-function-argument, missing-return-type-private-function]
        return merge_present_dict_values(current, incoming)

    coordinator._merge_partial_device_update = MagicMock(side_effect=mock_merge)  # ruff: ignore[private-member-access]

    merged = coordinator._merge_concurrent_coordinator_updates(baseline, result)  # ruff: ignore[private-member-access]

    # Layer-5 delta for some_stat should be preserved
    assert merged["device1"]["some_stat"] == 105  # ruff: ignore[magic-value-comparison]


async def test_shadow_queries_do_not_block_primary_http() -> None:  # ruff: ignore[unused-async]
    ...
    from types import SimpleNamespace  # ruff: ignore[import-outside-top-level]
    from unittest.mock import Mock  # ruff: ignore[import-outside-top-level]
    ...
    coordinator._cached_date = None  # ruff: ignore[private-member-access]
    ...
    cast("Any", coordinator)._async_query_all_properties_for_device = AsyncMock(return_value={})  # ruff: ignore[private-member-access]
    coordinator._async_query_system_info_for_missing = AsyncMock()  # ruff: ignore[private-member-access]
    coordinator._async_refresh_discovery_if_due = AsyncMock()  # ruff: ignore[private-member-access]
    ...
    assert callable(coordinator._schedule_background_once)  # ruff: ignore[private-member-access]


async def test_fresh_http_data_overwrites_shadow_in_merge() -> None:  # ruff: ignore[unused-async]
...
async def test_poll_watchdog_uses_configured_interval() -> None:  # ruff: ignore[unused-async]
...
async def test_merge_handles_missing_property_source_state() -> None:  # ruff: ignore[unused-async]
    ...
    assert "device1" in merged
    assert merged["device1"][PAYLOAD_PROPERTIES]["batSoc"] == 52  # ruff: ignore[magic-value-comparison]
