...
class TestLocalPeriodTotalSupersedesCloud:
    ...
    def test_local_none_returns_false(self) -> None:
        assert local_period_total_supersedes_cloud(10.0, None) is False

    def test_local_zero_returns_false(self) -> None:
        assert local_period_total_supersedes_cloud(10.0, 0.0) is False

    def test_local_negative_returns_false(self) -> None:
        assert local_period_total_supersedes_cloud(10.0, -1.0) is False

    def test_cloud_none_with_positive_local_returns_true(self) -> None:
        assert local_period_total_supersedes_cloud(None, 5.0) is True

    def test_local_exceeds_cloud_plus_tolerance(self) -> None:
...
class TestIsPeriodicSection:
    ...
    def test_bare_prefixes_are_periodic(self, section_key: str) -> None:
        assert is_periodic_section(section_key) is True

    def test_date_type_suffix_is_periodic(self) -> None:
    ...
    def test_properties_is_not_periodic(self) -> None:
        assert is_periodic_section(PAYLOAD_PROPERTIES) is False

    def test_unrelated_key_is_not_periodic(self) -> None:
...
class TestMergeLiveProperties:
    ...
    def test_update_wins_for_populated_values(self) -> None:
        base = {"pvPw": 100, "soc": 50}
        update = {"pvPw": 120}
        merged = merge_live_properties(base, update)
        assert merged["pvPw"] == 120
        assert merged["soc"] == 50

    def test_none_does_not_blank_populated_value(self) -> None:
        base = {"pvPw": 100}
        update = {"pvPw": None}
        assert merge_live_properties(base, update)["pvPw"] == 100

    def test_empty_string_does_not_blank_populated_value(self) -> None:
        base = {"name": "SolarVault"}
        update = {"name": "   "}
        assert merge_live_properties(base, update)["name"] == "SolarVault"

    def test_empty_list_does_not_blank_populated_value(self) -> None:
        base = {"modes": ["auto"]}
        update = {"modes": []}
        assert merge_live_properties(base, update)["modes"] == ["auto"]

    def test_empty_dict_does_not_blank_populated_value(self) -> None:
        base = {"config": {"key": "val"}}
        update = {"config": {}}
        assert merge_live_properties(base, update)["config"] == {"key": "val"}

    def test_none_can_fill_previously_none_field(self) -> None:
    ...
    def test_recursive_none_in_nested_does_not_blank(self) -> None:
        base = {"nested": {"a": 1}}
    ...
    def test_new_fields_are_added(self) -> None:
...
class TestIngestNestedProvenancePaths:
    ...
    def test_keep_current_supplements_sparse_dict(self) -> None:
        ...
        assert http.payload["nested"]["b"] == 2

    def test_scalar_replaces_dict_value(self) -> None:
        ...
        assert second.payload["field"] == 42
    ...
    def test_dict_replaces_scalar_value(self) -> None:
    ...
    def test_drop_descendant_provenance_on_shape_change(self) -> None:
    ...
    def test_nested_accepted_propagates_up(self) -> None:
        ...
        assert second.payload["top"]["inner"] == 2

    def test_nested_not_accepted_skips_parent_update(self) -> None:
    ...
    def test_received_at_falls_back_to_observation(self) -> None:
        ...
        assert result.provenance["v"].received_at_monotonic == 200.0

    def test_received_at_falls_back_to_time_monotonic(self) -> None:
