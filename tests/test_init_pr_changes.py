...
The integration is imported directly; an import failure surfaces as a hard
test error so syntax/dependency regressions are impossible to miss.
"""
...
def _make_stub_coordinator() -> Any:  # noqa: ANN401
    """Return a bare coordinator instance for _async_start_local_mqtt calls.

    `_async_start_local_mqtt` only references the coordinator from inside the
    never-invoked local MQTT message sink, so an uninitialised instance built
    via `__new__` (mirroring test_ble_frame.py) is sufficient.
    """
...
async def test_async_start_local_mqtt_does_nothing_when_disabled() -> None:
    ...
    await _async_start_local_mqtt(hass, entry, _make_stub_coordinator())
...
async def test_async_start_local_mqtt_unload_callback_stops_and_removes_client() -> (
    ...
    with patch.object(client, "async_stop", new=_fake_stop):
        ...
        callback()
        await asyncio.gather(*hass._bg_tasks)  # noqa: SLF001
