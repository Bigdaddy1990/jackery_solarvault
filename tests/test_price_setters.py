...
class _RejectingPriceApi:
    async def async_set_single_mode(  # ruff: ignore[no-self-use]
        self,
        *,
        system_id: str,
        single_price: float,
    ...
    async def async_set_dynamic_mode(  # ruff: ignore[no-self-use]
        self,
        *,
        system_id: str,
...
def _coordinator() -> JackerySolarVaultCoordinator:
    ...
    coordinator.data = {
        "dev1": {
            ...
            # dev1 as such so _require_home_config_context lets the write through
            # to the (rejecting) API rather than aborting on device type.
            PAYLOAD_SYSTEM: {FIELD_SYSTEM_ID: "sys1"},
            PAYLOAD_PRICE: {
                FIELD_CURRENCY: "EUR",
