...
class TestMergeIdentifiedDictListsUncovered:
    ...
    def test_returns_none_for_unidentified_lists(self) -> None:  # ruff: ignore[no-self-use]
    ...
    def test_appends_new_items_when_base_empty(self) -> None:  # ruff: ignore[no-self-use]
    ...
    def test_returns_none_for_empty_updates(self) -> None:  # ruff: ignore[no-self-use]
...
class TestMergePresentDictValuesUncovered:
    ...
    def test_list_with_identified_items(self) -> None:  # ruff: ignore[no-self-use]
...
class TestFindListForKeyUncovered:
    ...
    def test_finds_list_in_dict_values(self) -> None:  # ruff: ignore[no-self-use]
    ...
    def test_finds_list_in_list_elements(self) -> None:  # ruff: ignore[no-self-use]
        """Test finds list in list elements (line 1689)."""
        obj = [{"nested": {"batteryPacks": [{"deviceSn": "p1"}]}}]
        result = find_list_for_key(obj, "batteryPacks")
        assert result == [{"deviceSn": "p1"}]

    def test_returns_none_for_string(self) -> None:  # ruff: ignore[no-self-use]
        """Test returns None for string input."""
        result = find_list_for_key("string", "batteryPacks")
        assert result is None

    def test_returns_none_for_int(self) -> None:  # ruff: ignore[no-self-use]
