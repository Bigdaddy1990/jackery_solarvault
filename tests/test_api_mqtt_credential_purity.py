...
class TestMqttCredentialCanonical:
    ...
    @pytest.mark.asyncio()
    async def test_async_get_mqtt_credentials_is_canonical(self) -> None:  # ruff: ignore[no-self-use]
    ...
    @pytest.mark.asyncio()
    async def test_get_cached_delegates_to_canonical(self) -> None:  # ruff: ignore[no-self-use]
        ...
        api._mqtt_user_id = "user123"  # ruff: ignore[private-member-access]
        api._mqtt_seed_b64 = base64.b64encode(b"x" * 32).decode("ascii")  # ruff: ignore[private-member-access]
        api._mqtt_mac_id = "2" + "a" * 32  # valid 33-char MAC ID  # ruff: ignore[private-member-access]
    ...
    @pytest.mark.asyncio()
    async def test_base64_decode_error_raises_typed_error_not_nameerror(self) -> None:  # ruff: ignore[no-self-use]
        """Base64 decoding failures return None, not NameError or unhandled exception."""  # ruff: ignore[line-too-long]
        api = _make_api()
        api._mqtt_user_id = "user123"  # ruff: ignore[private-member-access]
        api._mqtt_seed_b64 = "not-valid-base64!!!"  # ruff: ignore[private-member-access]
        api._mqtt_mac_id = "2" + "a" * 32  # ruff: ignore[private-member-access]
    ...
    @pytest.mark.asyncio()
    async def test_base64_decode_wrong_length_returns_none(self) -> None:  # ruff: ignore[no-self-use]
    ...
    @pytest.mark.asyncio()
    async def test_no_plaintext_token_in_logs(self) -> None:  # ruff: ignore[no-self-use]
        ...
        # Capture log output
        from io import StringIO  # ruff: ignore[import-outside-top-level]
        ...
        try:
            ...
            assert api._mqtt_seed_b64 not in log_output, "Seed must not appear in logs"  # ruff: ignore[private-member-access]
            assert api._mqtt_mac_id not in log_output, "MAC ID must not appear in logs"  # ruff: ignore[private-member-access]
        finally:
    ...
    async def test_no_direct_decoding_duplication_in_rsa_encrypt(self) -> None:  # ruff: ignore[no-self-use]
        ...
        # Valid base64 but invalid DER should raise ValueError (from load_der_public_key)  # ruff: ignore[line-too-long]
    ...
    @pytest.mark.asyncio()
    async def test_mqtt_credential_structure_is_complete(self) -> None:  # ruff: ignore[no-self-use]
