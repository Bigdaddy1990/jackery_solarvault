...
MQTT_IMPLEMENTATION = CLIENT_PACKAGE / "mqtt" / "mqtt_push.py"
...
def _class_constant_int(tree: ast.Module, class_name: str, attr_name: str) -> int:
    ...
    raise AssertionError(f"Missing {class_name}.{attr_name} integer constant")  # noqa: TRY003
...
def test_unredacted_diagnostics_option_is_options_flow_only() -> None:
    ...
    # The option default is declared centrally in const.py's _OPTION_DEFAULTS
    # map and config_flow seeds option values from that map via a
    # ``{**_OPTION_DEFAULTS}`` spread, so the literal pairing lives in const.py
    # (not as inline text in config_flow). Assert the real invariant: the
    # default pairing exists in _OPTION_DEFAULTS.
    option_defaults_block = const_source.split(
        "_OPTION_DEFAULTS: dict[str, bool] = {",
        1,
    )[1].split("}", 1)[0]
    assert (
        "CONF_ENABLE_UNREDACTED_DIAGNOSTICS: DEFAULT_ENABLE_UNREDACTED_DIAGNOSTICS"
        in option_defaults_block
    )
    ...
    # The unsafe raw-diagnostics switch must stay out of the initial user setup
    # step and only be reachable from the options flow.
...
def test_services_yaml_matches_registered_services_and_validates_numeric_ids() -> None:
    ...
    for service in (
        ...
        "send_ble_command",
        "list_shared_devices",
        "list_shared_managers",
        "remove_shared_access",
        "remove_all_shared_access",
    ):
    ...
    assert set(services["list_shared_devices"]["fields"]) == {"device_id"}
    assert set(services["list_shared_managers"]["fields"]) == {
        "device_id",
        "bind_user_id",
        "level",
    }
    assert set(services["remove_shared_access"]["fields"]) == {
        "device_id",
        "bind_user_id",
    }
    assert set(services["remove_all_shared_access"]["fields"]) == {
        "device_id",
        "bind_user_id",
        "level",
    }
    ...
    # P2 sharing read services return data via SupportsResponse.ONLY; the table
    # carries supports_response and the read handlers are typed -> ServiceResponse.
    assert "SupportsResponse.ONLY" in services_source
    assert "ServiceResponse" in services_source
    assert "supports_response=registration.supports_response" in services_source
    ...
    # in services.yaml can hand them through unchanged. ruff wraps the
    # ``vol.All(...)`` across lines, so collapse whitespace before matching.
    services_source_collapsed = re.sub(r"\s+", " ", services_source)
    assert (
        "cv.string, vol.Match(SERVICE_NUMERIC_ID_PATTERN)" in services_source_collapsed
    )
...
def test_all_api_json_decode_paths_catch_value_error() -> None:
    ...
    assert len(decode_blocks) == 5, decode_blocks  # noqa: PLR2004
...
def test_coordinator_lazy_imports_mqtt_client_for_collection_without_aiomqtt() -> None:
    ...
    module_mqtt_imports = [
        ...
        and node.module == "client.mqtt.mqtt_push"
    ...
    assert 'importlib.import_module(".client.mqtt.mqtt_push", __package__)' in source
...
def test_services_setup_is_idempotent_and_callback_typed() -> None:
    ...
    # The @callback decorator must immediately precede the def. Services are
    # registered from a declarative table (_service_registrations) iterated in
    # async_setup_services, so the per-service consts live in that table rather
    # than inline in the setup body.
    assert "@callback\ndef async_setup_services" in services_source
    registration_table = services_source.split("def _service_registrations", 1)[
        1
    ].split(
        "\n@callback",
        1,
    ...
    for service_const in (
        ...
        assert service_const in registration_table, service_const
    # Re-entry safe: HA fires async_setup multiple times in some test setups,
    # so each registration is gated on has_service before async_register.
    assert "if not hass.services.has_service(DOMAIN, registration.name):" in setup_block
    assert "hass.services.async_register(" in setup_block
    ...
    for path in _python_sources():
        ...
        for owner in ast.walk(tree):
            ...
            for call in ast.walk(owner):
                ...
                assert not (too_few or too_many), (  # noqa: PT018
...
def test_sensor_setup_uses_shared_bool_option_fallback_helper() -> None:
    ...
    assert setup_block.count("config_entry_bool_option(") == 3  # noqa: PLR2004
...
def test_no_unresolved_git_merge_conflict_markers() -> None:
    ...
    marker_prefixes = ("<<<<<<< ", ">>>>>>> ")
    scan_roots = (CUSTOM_COMPONENT, pathlib.Path("tests"))
...
def test_init_annotations_are_safe_after_pre_commit_autofix() -> None:
    ...
    # TYPE_CHECKING is permitted in __init__.py: it guards type-only imports
    # so HA collection never executes them at runtime. The invariant we care
    # about is that every `if TYPE_CHECKING:` block contains only imports (no
    # runtime code), not that TYPE_CHECKING is absent.
    for node in ast.walk(init_tree):
        ...
        ), "TYPE_CHECKING block in __init__.py must contain only imports"
...
def test_mqtt_tls_uses_verified_jackery_ca_without_insecure_fallback() -> None:
    ...
    # SSL context is built from Python's verified default context (the
    # ``purpose=`` kwarg is permitted) and never weakened. Verification stays
    # on (CERT_REQUIRED + check_hostname) with no insecure escape hatch.
    assert "ssl.create_default_context(" in mqtt_source
    assert "ctx.verify_mode = ssl.CERT_REQUIRED" in mqtt_source
    assert "ctx.check_hostname = True" in mqtt_source
    assert "ctx.check_hostname = False" not in combined
    ...
    # The MQTT diagnostics snapshot reports TLS as never-insecure.
    assert '"tls_insecure": False' in mqtt_source
    # The coordinator diagnostics block reports certificate verification as
    # unconditionally enabled with no insecure-TLS warning. (The previous
    # multi-line conditional that derived "chain_hostname_enabled[...]"
    # descriptors was flattened; the invariant is that verification is
    # always reported on and the insecure-warning slot stays empty.)
    assert 'diag["tls_certificate_verification"] = "enabled"' in coordinator_source
    assert 'diag["tls_insecure_warning"] = None' in coordinator_source
...
def test_ble_sink_calls_merge_with_correct_signature() -> None:
    ...
    assert arg_commas >= 2, (  # noqa: PLR2004
...
def test_historical_statistics_backfill_is_http_only() -> None:
    """Historical statistic backfill must stay HTTP-driven, never MQTT-driven.

    The calendar-iteration helpers (``_iter_calendar_*``) and the app-chart
    fetchers are live, documented HTTP code that walks completed-day curves via
    ``self.api.async_get_*`` cloud calls. The invariant under test is the
    *source* of the backfill data: it must come from those HTTP endpoints, and
    there must be no MQTT-push-driven backfill scheduler.
    """
    source = (CUSTOM_COMPONENT / "coordinator.py").read_text(encoding="utf-8")

    # The HTTP backfill entrypoints exist and pull data over HTTP.
    ...
    # The day-chart fetcher is genuinely HTTP-backed: it sources every section
    # from the cloud API (``self.api.async_get_*``), not from MQTT state.
    day_fetch_block = source.split(
        "async def _async_fetch_historical_day_chart_sources",
        1,
    )[1].split("\n    async def ", 1)[0]
    assert "self.api.async_get_device_battery_stat(" in day_fetch_block
    assert "self.api.async_get_device_home_stat(" in day_fetch_block

    # The window is bounded (7 completed days) and throttled on a fixed
    # interval — both sourced from named constants, not MQTT cadence.
    assert "STATISTICS_HTTP_BACKFILL_WINDOW_DAYS" in source
    assert "_STATISTICS_HTTP_BACKFILL_INTERVAL_SEC" in source

    # No MQTT-push-driven backfill scheduler may exist.
