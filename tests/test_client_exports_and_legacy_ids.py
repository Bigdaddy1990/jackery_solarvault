...
- __init__.py new import path: JackeryLocalMqttClient is imported from
  .client.mqtt.local_mqtt (new import added for this integration).
...
from custom_components.jackery_solarvault.client.api import (
    JackeryApiError,
    JackeryAuthError,  # noqa: F811
    JackeryError,  # noqa: F811
)
...
# ---------------------------------------------------------------------------
# __init__.py — new import path for JackeryLocalMqttClient (.client.mqtt.local_mqtt)
# ---------------------------------------------------------------------------
...
def test_init_imports_jackery_local_mqtt_client() -> None:
    ...
    The package exposes
    ``from .client.mqtt.local_mqtt import JackeryLocalMqttClient`` to __init__.py.
...
def test_client_init_imports_jackery_api_from_client_package() -> None:
    """JackeryApi must be importable from the client sub-package.
...
def test_legacy_suffix_matches_head_cannot_have_trailing_underscore() -> None:
    """A digits head with a trailing underscore before the suffix is fine if suffix.
...
def test_local_mqtt_result_warning_condition_fires_for_runtime_error() -> None:
    """The isinstance(local_mqtt_result, BaseException) condition must be True for.
