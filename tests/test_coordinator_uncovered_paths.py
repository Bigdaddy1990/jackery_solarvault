...
class TestNormalizeBackfillStatus:
    ...
    def test_known_status_returns_enum(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        result = _normalize_backfill_status(BackfillStatus.IMPORTED, closed=True)
        assert result == BackfillStatus.IMPORTED

    def test_auth_error_maps_to_retryable(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        result = _normalize_backfill_status("auth_error", closed=True)
        assert result == BackfillStatus.RETRYABLE

    def test_unknown_open_maps_to_pending(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
    ...
    def test_invalid_type_returns_pending(self) -> None:  # ruff: ignore[no-self-use]
...
class TestChangedDictValues:
    ...
    def test_detects_new_keys(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        before = {"a": 1}
        after = {"a": 1, "b": 2}
        result = changed_dict_values(before, after)
        assert result == {"b": 2}

    def test_detects_changed_values(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        before = {"a": 1}
        after = {"a": 2}
        result = changed_dict_values(before, after)
        assert result == {"a": 2}

    def test_handles_nested_dicts(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        before = {"device": {"soc": 50}}
        after = {"device": {"soc": 60}}
        result = changed_dict_values(before, after)
        assert result == {"device": {"soc": 60}}

    def test_returns_empty_for_no_changes(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        before = {"a": 1}
        after = {"a": 1}
        result = changed_dict_values(before, after)
        assert result == {}
...
class TestMergePresentDictValuesAdditional:
    ...
    def test_preserves_base_when_update_blank(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        base = {"key": "value"}
        updates = {"key": None}
        result = merge_present_dict_values(base, updates)
        assert result["key"] == "value"

    def test_update_overwrites_when_base_blank(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        base = {"key": None}
...
class TestMergeMissingDictValuesAdditional:
    ...
    def test_fills_missing_nested(self) -> None:  # ruff: ignore[undocumented-public-method, no-self-use]
        base = {"device": {"soc": 50}}
