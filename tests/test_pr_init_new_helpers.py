...
- _async_authenticate_api_layer: auth success; auth failure (ConfigEntryAuthFailed);
  transient cloud failure (UpdateFailed, since HTTP is the primary unconditional path)
...
def _fake_hass() -> HomeAssistant:
    """Return a _FakeHass typed as HomeAssistant for the production helpers."""
    return cast("HomeAssistant", _FakeHass())
...
def _fake_entry(
    ...
    """Return a _FakeEntry typed as ConfigEntry for the production helpers."""
    return cast(
        "ConfigEntry",
        _FakeEntry(entry_id=entry_id, options=options, data=data),
    )
...
def _make_local_mqtt_entry(  # noqa: PLR0913
    *,
    ...
    """Return a _FakeEntry (typed as ConfigEntry) configured with CONF_LOCAL_MQTT_*."""
    return _fake_entry(
        entry_id=entry_id,
        options={
...
class TestEntryBootstrapMqttSession:
    ...
    def test_returns_none_when_bootstrap_value_is_not_dict(self) -> None:  # noqa: PLR6301
        ...
        entry = _fake_entry(data={ENTRY_BOOTSTRAP_MQTT_SESSION: "not-a-dict"})
        result = _entry_bootstrap_mqtt_session(entry)
        assert result is None

    def test_returns_none_when_bootstrap_value_is_none(self) -> None:  # noqa: PLR6301
        """Returns None when the bootstrap value is None."""
        entry = _fake_entry(data={ENTRY_BOOTSTRAP_MQTT_SESSION: None})
        result = _entry_bootstrap_mqtt_session(entry)
        assert result is None
...
class TestEntryRuntimeBucket:
    ...
    def test_separate_entries_get_separate_buckets(self) -> None:  # noqa: PLR6301
        ...
        hass = _fake_hass()
        entry_a = _fake_entry(entry_id="ea")
        entry_b = _fake_entry(entry_id="eb")
...
class TestAsyncStartLocalMqttNewKeys:
    ...
    async def test_empty_topic_filter_prevents_client_creation(self) -> None:  # noqa: PLR6301
        ...
        hass = _fake_hass()
        entry = _make_local_mqtt_entry(
            enable=True, host="192.168.1.200", topic_filter=""
        )
        coordinator = MagicMock()
    ...
    async def test_scoped_filter_is_not_blocked(self) -> None:  # noqa: PLR6301
        ...
        hass = _fake_hass()
        entry = _make_local_mqtt_entry(
            enable=True, host="192.168.1.200", topic_filter="hb/app/+/device"
        )
        coordinator = MagicMock()
        coordinator.async_handle_local_mqtt_message = AsyncMock()
    ...
    async def test_client_id_uses_first_8_chars_of_entry_id(self) -> None:  # noqa: PLR6301
        ...
        entry = _make_local_mqtt_entry(
            enable=True, host="192.168.1.200", entry_id=entry_id
        )
        coordinator = MagicMock()
        coordinator.async_handle_local_mqtt_message = AsyncMock()
    ...
    async def test_topic_filter_is_passed_to_client(self) -> None:  # noqa: PLR6301
        ...
        hass = _fake_hass()
        entry = _make_local_mqtt_entry(
            enable=True, host="192.168.1.200", topic_filter="sv3pro/+/status"
        )
        coordinator = MagicMock()
        coordinator.async_handle_local_mqtt_message = AsyncMock()
    ...
    async def test_unload_callback_registered(self) -> None:  # noqa: PLR6301
        ...
        assert len(cast("_FakeEntry", entry)._unload_callbacks) == 1  # noqa: SLF001
    ...
    async def test_unload_callback_stops_and_removes_client(self) -> None:  # noqa: PLR6301
        ...
        with patch.object(client, "async_stop", new=_fake_stop):
            cast("_FakeEntry", entry)._unload_callbacks[0]()  # noqa: SLF001
            await asyncio.gather(*cast("_FakeHass", hass)._bg_tasks)  # noqa: SLF001
...
class TestAsyncAuthenticateApiLayer:
    ...
    async def test_raises_update_failed_on_jackery_error_transient(  # noqa: PLR6301
        self,
    ) -> None:
        """A transient JackeryError must raise UpdateFailed.

        HTTP/cloud login is the primary, unconditional data path. When it is
        unavailable, Layer 5 startup is blocked until HTTP login succeeds, so the
        production helper re-raises the transient failure as UpdateFailed rather
        than swallowing it (see __init__.py:_async_authenticate_api_layer).
        """
        from custom_components.jackery_solarvault import (  # noqa: I001, PLC0415
            _async_authenticate_api_layer,  # noqa: PLC2701
        )
        ...
        with (
            ...
            pytest.raises(UpdateFailed, match="login is unavailable"),
        ):
            await _async_authenticate_api_layer(hass, entry, api)
