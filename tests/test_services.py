...
# ---------------------------------------------------------------------------
# P1 device-operation services: set_device_nickname, unbind_device,
# bind_smart_part, unbind_smart_part
# ---------------------------------------------------------------------------
...
class _NicknameApi:
    ...
    async def async_set_device_nickname(
        ...
        self.calls.append((device_id, nickname))
        return {}


class _NicknameCoordinator:
    def __init__(self) -> None:
        self.api = _NicknameApi()
        self.refreshed = False
...
class _UnbindApi:
    ...
    async def async_unbind_device(self, device_id: str) -> dict[str, object]:
        self.calls.append(device_id)
        return {}


class _UnbindCoordinator:
    def __init__(self) -> None:
        self.api = _UnbindApi()
        self.refreshed = False
...
class _SmartPartCoordinator:
    ...
    async def async_bind_smart_part(self, device_id: str, accessory_sn: str) -> None:
        self.bind_calls.append((device_id, accessory_sn))

    async def async_unbind_smart_part(self, device_id: str, accessory_sn: str) -> None:
        self.unbind_calls.append((device_id, accessory_sn))


async def test_set_device_nickname_service_forwards_resolved_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Nickname service forwards the resolved Jackery id to the API client."""
    coordinator = _NicknameCoordinator()
    monkeypatch.setattr(
        services,
        "_resolve_jackery_device_id",
        lambda _hass, _raw: "573702884982521856",
    )
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    await services._async_handle_set_device_nickname(  # noqa: SLF001
        _hass(),
        _call({
            SERVICE_FIELD_DEVICE_ID: "ha-uuid",
            SERVICE_FIELD_NICKNAME: " Balcony ",
        }),
    )

    assert coordinator.api.calls == [("573702884982521856", "Balcony")]


async def test_set_device_nickname_service_rejects_unknown_device(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Nickname service raises a translated error when no entry owns the device."""
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
        lambda _hass, _device_id: None,
    )
    ...
    assert err.value.translation_key == "set_device_nickname_failed"
    assert err.value.translation_placeholders == {
        "device_id": "dev1",
...
async def test_set_device_nickname_service_reauth_on_auth_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Nickname service preserves auth failures so HA can open reauth."""

    class _AuthNicknameApi:
        async def async_set_device_nickname(self, *args: object) -> dict[str, object]:  # noqa: PLR6301
            msg = "invalid token"
            raise JackeryAuthError(msg)

    class _AuthNicknameCoordinator:
        api = _AuthNicknameApi()

    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
        lambda _hass, _device_id: _AuthNicknameCoordinator(),
    )
...
async def test_set_device_nickname_service_rejects_blank_nickname(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Direct nickname calls keep the non-empty constraint before the API call."""

    class _GuardCoordinator:
        class _GuardApi:
            async def async_set_device_nickname(  # noqa: PLR6301
                self,
                *args: object,
            ) -> dict[str, object]:
                msg = "blank nickname must stop before api call"
                raise AssertionError(msg)
    ...
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
        lambda _hass, _device_id: _GuardCoordinator(),
    )
...
async def test_unbind_device_service_refreshes_after_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unbind forwards the resolved id and refreshes so the device drops."""
    coordinator = _UnbindCoordinator()
    monkeypatch.setattr(
        services,
        "_resolve_jackery_device_id",
        lambda _hass, _raw: "573702884982521856",
    ...
    await services._async_handle_unbind_device(  # noqa: SLF001
        _hass(),
        _call({SERVICE_FIELD_DEVICE_ID: "ha-uuid"}),
    )

    assert coordinator.api.calls == ["573702884982521856"]
    assert coordinator.refreshed is True


async def test_unbind_device_service_rejects_unknown_device(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unbind raises a translated error when no entry owns the device."""
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    assert err.value.translation_key == "unbind_device_failed"
    assert err.value.translation_placeholders == {
        "device_id": "dev1",
        "error": "no Jackery entry owns this device id",
    }
...
async def test_unbind_device_service_reauth_on_auth_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unbind preserves auth failures and never refreshes on auth error."""

    class _AuthUnbindApi:
        async def async_unbind_device(self, *args: object) -> dict[str, object]:  # noqa: PLR6301
            msg = "invalid token"
            raise JackeryAuthError(msg)

    class _AuthUnbindCoordinator:
        api = _AuthUnbindApi()
    ...
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
        lambda _hass, _device_id: _AuthUnbindCoordinator(),
    )
...
async def test_bind_smart_part_service_forwards_resolved_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Bind smart-part forwards the resolved id and accessory SN to the coordinator."""
    coordinator = _SmartPartCoordinator()
    monkeypatch.setattr(
        services,
        "_resolve_jackery_device_id",
        lambda _hass, _raw: "573702884982521856",
    ...
    await services._async_handle_bind_smart_part(  # noqa: SLF001
        _hass(),
        _call({
            SERVICE_FIELD_DEVICE_ID: "ha-uuid",
            SERVICE_FIELD_ACCESSORY_SN: " SN123 ",
        }),
    )

    assert coordinator.bind_calls == [("573702884982521856", "SN123")]


async def test_bind_smart_part_service_rejects_unknown_device(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Bind smart-part raises a translated error for an unowned device."""
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    assert err.value.translation_key == "bind_smart_part_failed"
    assert err.value.translation_placeholders == {
        "device_id": "dev1",
        "error": "no Jackery entry owns this device id",
    }
...
async def test_bind_smart_part_service_reauth_on_auth_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Bind smart-part preserves auth failures so HA can open reauth."""

    class _AuthSmartPartCoordinator:
        async def async_bind_smart_part(self, *args: object) -> None:  # noqa: PLR6301
    ...
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
        lambda _hass, _device_id: _AuthSmartPartCoordinator(),
    )
...
async def test_unbind_smart_part_service_forwards_resolved_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unbind smart-part forwards the resolved id and accessory SN."""
    coordinator = _SmartPartCoordinator()
    monkeypatch.setattr(
        services,
        "_resolve_jackery_device_id",
    ...
    await services._async_handle_unbind_smart_part(  # noqa: SLF001
        _hass(),
        _call({
            SERVICE_FIELD_DEVICE_ID: "ha-uuid",
            SERVICE_FIELD_ACCESSORY_SN: "SN123",
    ...
    assert coordinator.unbind_calls == [("573702884982521856", "SN123")]


async def test_unbind_smart_part_service_rejects_blank_accessory_sn(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Direct unbind smart-part calls keep the non-empty accessory_sn constraint."""

    class _GuardCoordinator:
        async def async_unbind_smart_part(self, *args: object) -> None:  # noqa: PLR6301
            msg = "blank accessory_sn must stop before coordinator call"
            raise AssertionError(msg)
    ...
    assert err.value.translation_key == "unbind_smart_part_failed"
    assert err.value.translation_placeholders == {
        "device_id": "dev1",
        "error": "accessory_sn must not be empty",
    }


async def test_unbind_smart_part_service_reauth_on_auth_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unbind smart-part preserves auth failures so HA can open reauth."""
...
# ---------------------------------------------------------------------------
# P2 sharing services: list_shared_devices, list_shared_managers,
# remove_shared_access, remove_all_shared_access
#
# All resolve the coordinator via a required device_id (device selector) for
# multi-account safety even though the underlying cloud call is account-level.
# The two read services use SupportsResponse.ONLY and return a dict envelope.
# ---------------------------------------------------------------------------


class _SharedListApi:
    def __init__(self, devices: list[dict[str, object]]) -> None:
        self._devices = devices
        self.calls = 0

    async def async_get_device_shared_list(self) -> list[dict[str, object]]:
        self.calls += 1
        return self._devices


class _SharedListCoordinator:
    def __init__(self, devices: list[dict[str, object]]) -> None:
        self.api = _SharedListApi(devices)


class _SharedManagersApi:
    def __init__(self, managers: list[dict[str, object]]) -> None:
    ...
    async def async_get_device_shared_managers(
        ...
        *,
        ...
        self.calls.append((bind_user_id, level))
        return self._managers


class _SharedManagersCoordinator:
    def __init__(self, managers: list[dict[str, object]]) -> None:
        self.api = _SharedManagersApi(managers)


class _RemoveSharedApi:
    def __init__(self) -> None:
        self.calls: list[tuple[str, str]] = []
    ...
    async def async_remove_shared_access(
        ...
        *,
        ...
        self.calls.append((bind_user_id, device_id))
        return {}


class _RemoveSharedCoordinator:
    def __init__(self) -> None:
        self.api = _RemoveSharedApi()
        self.refreshed = False
...
class _RemoveAllSharedCoordinator:
    def __init__(self) -> None:
        self.api = _RemoveAllSharedApi()
        self.refreshed = False
...
def test_list_shared_devices_registration_uses_supports_response_only() -> None:
    """The read sharing services are registered with SupportsResponse.ONLY."""
    from homeassistant.core import SupportsResponse  # noqa: PLC0415

    registrations = {
        registration.name: registration
        for registration in services._service_registrations()  # noqa: SLF001
    }

    assert (
        registrations["list_shared_devices"].supports_response is SupportsResponse.ONLY
    )
    assert (
        registrations["list_shared_managers"].supports_response is SupportsResponse.ONLY
    )
    assert (
        registrations["remove_shared_access"].supports_response is SupportsResponse.NONE
    )
    assert (
        registrations["remove_all_shared_access"].supports_response
        is SupportsResponse.NONE
    )
    assert registrations["unbind_device"].supports_response is SupportsResponse.NONE


async def test_list_shared_devices_service_returns_device_envelope(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """list_shared_devices returns the API list under a devices key."""
    devices = [{"deviceId": "1"}, {"deviceId": "2"}]
    coordinator = _SharedListCoordinator(devices)
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    result = await services._async_handle_list_shared_devices(  # noqa: SLF001
        _hass(),
        _call({SERVICE_FIELD_DEVICE_ID: "dev1"}),
    )
...
async def test_list_shared_devices_service_rejects_unknown_device(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """list_shared_devices raises a translated error for an unowned device."""
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    with pytest.raises(ServiceValidationError) as err:
        await services._async_handle_list_shared_devices(  # noqa: SLF001
            _hass(),
            _call({SERVICE_FIELD_DEVICE_ID: "dev1"}),
        )

    assert err.value.translation_key == "list_shared_devices_failed"
    assert err.value.translation_placeholders == {
        "device_id": "dev1",
        "error": "no Jackery entry owns this device id",
    }
...
async def test_list_shared_devices_service_reauth_on_auth_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """list_shared_devices preserves auth failures so HA can open reauth."""

    class _AuthSharedListApi:
        async def async_get_device_shared_list(self) -> list[dict[str, object]]:  # noqa: PLR6301
            msg = "invalid token"
            raise JackeryAuthError(msg)

    class _AuthSharedListCoordinator:
        api = _AuthSharedListApi()

    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
        lambda _hass, _device_id: _AuthSharedListCoordinator(),
    )
...
async def test_list_shared_managers_service_returns_manager_envelope(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """list_shared_managers forwards bind_user_id/level and returns the managers."""
    managers = [{"userId": "u1"}]
    coordinator = _SharedManagersCoordinator(managers)
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    result = await services._async_handle_list_shared_managers(  # noqa: SLF001
        _hass(),
        _call({
            SERVICE_FIELD_DEVICE_ID: "dev1",
            SERVICE_FIELD_BIND_USER_ID: " owner42 ",
            SERVICE_FIELD_LEVEL: 2,
        }),
    )

    assert coordinator.api.calls == [("owner42", 2)]
    assert result == {"managers": managers}


async def test_list_shared_managers_service_defaults_level_to_zero(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """list_shared_managers defaults the level filter to 0 when omitted."""
    coordinator = _SharedManagersCoordinator([])
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    assert coordinator.api.calls == [("owner42", 0)]
    assert result == {"managers": []}


async def test_list_shared_managers_service_rejects_blank_bind_user_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Blank bind_user_id is rejected before the API call."""

    class _GuardCoordinator:
        class _GuardApi:
            async def async_get_device_shared_managers(  # noqa: PLR6301
                self,
                **_kwargs: object,
            ) -> list[dict[str, object]]:
                msg = "blank bind_user_id must stop before api call"
                raise AssertionError(msg)
    ...
    with pytest.raises(ServiceValidationError) as err:
        await services._async_handle_list_shared_managers(  # noqa: SLF001
            _hass(),
            _call({
                SERVICE_FIELD_DEVICE_ID: "dev1",
                SERVICE_FIELD_BIND_USER_ID: "   ",
    ...
    assert err.value.translation_key == "list_shared_managers_failed"
    assert err.value.translation_placeholders == {
        "device_id": "dev1",
        "error": "bind_user_id must not be empty",
    }


async def test_remove_shared_access_service_refreshes_after_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """remove_shared_access forwards the resolved id and refreshes on success."""
    coordinator = _RemoveSharedCoordinator()
    monkeypatch.setattr(
        services,
        "_resolve_jackery_device_id",
        lambda _hass, _raw: "573702884982521856",
    ...
    await services._async_handle_remove_shared_access(  # noqa: SLF001
        _hass(),
        _call({
            SERVICE_FIELD_DEVICE_ID: "ha-uuid",
            SERVICE_FIELD_BIND_USER_ID: " owner42 ",
    ...
    assert coordinator.api.calls == [("owner42", "573702884982521856")]
    assert coordinator.refreshed is True


async def test_remove_shared_access_service_rejects_unknown_device(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """remove_shared_access raises a translated error for an unowned device."""
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    assert err.value.translation_key == "remove_shared_access_failed"
    assert err.value.translation_placeholders == {
        "device_id": "dev1",
        "error": "no Jackery entry owns this device id",
    }
...
async def test_remove_shared_access_service_rejects_blank_bind_user_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Blank bind_user_id is rejected before the destructive API call."""

    class _GuardCoordinator:
        class _GuardApi:
            async def async_remove_shared_access(  # noqa: PLR6301
                self,
                **_kwargs: object,
            ) -> dict[str, object]:
                msg = "blank bind_user_id must stop before api call"
        ...
        async def async_request_refresh(self) -> None:  # noqa: PLR6301
            msg = "validation failures must not refresh"
            raise AssertionError(msg)
...
async def test_remove_shared_access_service_reauth_never_refreshes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """remove_shared_access preserves auth failures and never refreshes on auth."""

    class _AuthRemoveSharedApi:
        async def async_remove_shared_access(  # noqa: PLR6301
            self,
            **_kwargs: object,
        ) -> dict[str, object]:
    ...
    class _AuthRemoveSharedCoordinator:
        api = _AuthRemoveSharedApi()
    ...
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
        lambda _hass, _device_id: _AuthRemoveSharedCoordinator(),
    )
...
async def test_remove_all_shared_access_service_refreshes_after_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """remove_all_shared_access forwards bind_user_id/level and refreshes on success."""
    coordinator = _RemoveAllSharedCoordinator()
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    await services._async_handle_remove_all_shared_access(  # noqa: SLF001
        _hass(),
        _call({
            SERVICE_FIELD_DEVICE_ID: "dev1",
            SERVICE_FIELD_BIND_USER_ID: "owner42",
            SERVICE_FIELD_LEVEL: 1,
        }),
    )

    assert coordinator.api.calls == [("owner42", 1)]
    assert coordinator.refreshed is True


async def test_remove_all_shared_access_service_defaults_level_to_zero(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """remove_all_shared_access defaults the level to 0 when omitted."""
    coordinator = _RemoveAllSharedCoordinator()
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
...
async def test_remove_all_shared_access_service_reauth_never_refreshes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """remove_all_shared_access preserves auth failures and never refreshes on auth."""

    class _AuthRemoveAllApi:
        async def async_remove_all_shared_access(  # noqa: PLR6301
            self,
            **_kwargs: object,
        ) -> dict[str, object]:
            msg = "invalid token"
    ...
    class _AuthRemoveAllCoordinator:
        api = _AuthRemoveAllApi()
    ...
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
        lambda _hass, _device_id: _AuthRemoveAllCoordinator(),
    )
...
# ---------------------------------------------------------------------------
# P3a sharing service: get_share_qr_code
#
# Mirrors the P2 read services: resolves the coordinator via a required
# device_id (device selector) for multi-account safety, uses
# SupportsResponse.ONLY, and returns a flat {qr_code_id, user_id} envelope.
# ---------------------------------------------------------------------------
...
class _QrCodeApi:
    ...
    async def async_get_qr_code(self) -> dict[str, object]:
        self.calls += 1
        return self._payload


class _QrCodeCoordinator:
    def __init__(self, payload: dict[str, object]) -> None:
        self.api = _QrCodeApi(payload)


def test_get_share_qr_code_registration_uses_supports_response_only() -> None:
    """get_share_qr_code is registered as a response-only read service."""
    from homeassistant.core import SupportsResponse  # noqa: PLC0415
    ...
    assert registrations["get_share_qr_code"].supports_response is SupportsResponse.ONLY


async def test_get_share_qr_code_service_returns_envelope(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """get_share_qr_code maps the cloud payload to the flat response envelope."""
    coordinator = _QrCodeCoordinator({"qrCodeId": "qr-77", "userId": "user-5"})
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    result = await services._async_handle_get_share_qr_code(  # noqa: SLF001
        _hass(),
        _call({SERVICE_FIELD_DEVICE_ID: "dev1"}),
    )

    assert coordinator.api.calls == 1
    assert result == {"qr_code_id": "qr-77", "user_id": "user-5"}


async def test_get_share_qr_code_service_creates_qr_notification(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The handler renders a QR image notification while still returning data."""
    ...
    def _capture(
        ...
        **_kwargs: object,
        ...
        created.append((message, title))

    monkeypatch.setattr(persistent_notification, "async_create", _capture)
    ...
    message, _title = created[0]
    assert "data:image/png;base64," in message
    assert "qr-77" in message


async def test_get_share_qr_code_service_survives_render_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A QR render failure must not fail the service or drop the response."""
    coordinator = _QrCodeCoordinator({"qrCodeId": "qr-77", "userId": "user-5"})
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
    ...
    def _boom(_qr_code_id: str) -> str:
        msg = "segno exploded"
        raise RuntimeError(msg)

    monkeypatch.setattr(services, "_render_share_qr_png_data_uri", _boom)
...
async def test_get_share_qr_code_service_rejects_unknown_device(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """get_share_qr_code raises a translated error for an unowned device."""
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
    ...
    with pytest.raises(ServiceValidationError) as err:
        await services._async_handle_get_share_qr_code(  # noqa: SLF001
            _hass(),
            _call({SERVICE_FIELD_DEVICE_ID: "dev1"}),
        )

    assert err.value.translation_key == "get_share_qr_code_failed"
    assert err.value.translation_placeholders == {
        "device_id": "dev1",
        "error": "no Jackery entry owns this device id",
    }
...
async def test_get_share_qr_code_service_maps_auth_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A raw JackeryAuthError surfaces as ConfigEntryAuthFailed (re-auth)."""

    class _AuthFailingCoordinator:
        def __init__(self) -> None:
            self.api = self

        async def async_get_qr_code(self) -> dict[str, object]:  # noqa: PLR6301
            msg = "rejected"
            raise JackeryAuthError(msg)

    coordinator = _AuthFailingCoordinator()
    monkeypatch.setattr(services, "_resolve_jackery_device_id", lambda _hass, raw: raw)
    monkeypatch.setattr(
        services,
        "_coordinator_for_device",
