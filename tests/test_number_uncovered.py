...
class TestMaxFeedGridAllowedValues:
    ...
    def test_under_800_returns_single(self) -> None:  # ruff: ignore[no-self-use]
...
class TestSingleTariffDynamicUnit:
    ...
    def test_single_currency(self) -> None:  # ruff: ignore[no-self-use]
        """Test single currency from price section."""
        payload = {"price": {"singleCurrency": "$"}}
        assert _single_tariff_dynamic_unit(payload) == "$"

    def test_currency(self) -> None:  # ruff: ignore[no-self-use]
        """Test currency from price section."""
        payload = {"price": {"currency": "€"}}
        assert _single_tariff_dynamic_unit(payload) == "€"

    def test_single_currency_code(self) -> None:  # ruff: ignore[no-self-use]
...
class TestJackeryNumber:
    ...
    def _create_number(self, coordinator: MagicMock, key="soc_charge_limit_set") -> JackeryNumber:  # ruff: ignore[no-self-use]
