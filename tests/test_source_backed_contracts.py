...
def test_all_source_truth_http_endpoints_exist_as_constants() -> None:
    ...
    assert expected_paths <= const_values  # noqa: S101
...
def test_http_endpoint_request_fields_are_not_doc_copies() -> None:
    ...
    assert by_path["auth/login"] == ["aesEncryptData", "rsaForAesKey"]  # noqa: S101
    assert by_path["device/bind/list"] == [""]  # noqa: S101
    assert by_path["device/property"] == ["deviceId"]  # noqa: S101
    assert by_path["device/dynamic/saveSingleMode"] == [  # noqa: S101
        "currency",
        "singlePrice",
        "systemId",
    ]
    assert by_path["device/accessories"] == ["devices", "id", "parentDeviceId"]  # noqa: S101
...
async def test_mqtt_command_envelope_matches_re_supplement() -> None:
    ...
    for key in (
        ...
        assert f'"{key}"' in supplement  # noqa: S101
    assert "hb/app/<sn>/command" in supplement  # noqa: S101
    ...
    assert published["topic"] == "hb/app/user1/command"  # noqa: S101
    assert set(published["payload"]) == {  # noqa: S101
        "id",
        FIELD_VERSION,
        FIELD_MESSAGE_TYPE,
        FIELD_ACTION_ID,
    ...
    assert published["payload"][FIELD_DEVICE_SN] == "sn1"  # noqa: S101
    assert published["payload"][FIELD_MESSAGE_TYPE] == "ControlCombine"  # noqa: S101
    assert published["payload"][FIELD_ACTION_ID] == 3027  # noqa: S101
    assert json.loads(published["payload"][FIELD_BODY]) == {"workModel": 1, "cmd": 121}  # noqa: S101
...
def test_crypto_layer_a_login_request_uses_source_truth_algorithms() -> None:
    ...
    assert "AES/ECB/PKCS5" in crypto  # noqa: S101
    assert "RSA/ECB/PKCS1" in crypto  # noqa: S101
    assert "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCV" in crypto  # noqa: S101

    plaintext = b'{"account":"u","password":"p"}'
    key = b"1234567890123456"
    ciphertext = _aes_ecb_encrypt(plaintext, key)
    assert ciphertext != plaintext  # noqa: S101
    assert len(ciphertext) % 16 == 0  # noqa: S101


def test_crypto_layer_b_mqtt_password_contract() -> None:
    """Layer B: password is AES-256-CBC(username, key=mqttPassWord, iv=key[:16])."""
    crypto = _read("Jackery_2.1.1_RE_Crypto_and_DTOs.md")
    assert "AES-256-CBC/PKCS5" in crypto  # noqa: S101
    assert "iv=key[:16]" in crypto  # noqa: S101
    ...
    assert _decrypt_aes_cbc(ciphertext, seed, seed[:16]) == username  # noqa: S101
    assert result[MQTT_CREDENTIAL_CLIENT_ID] == "12345@APP"  # noqa: S101
    assert result[MQTT_CREDENTIAL_USERNAME] == username.decode()  # noqa: S101
...
def test_crypto_layer_c_mqtt_body_contract() -> None:
    ...
    crypto = _read("Jackery_2.1.1_RE_Crypto_and_DTOs.md")
    assert "AES-128-CBC/PKCS7" in crypto  # noqa: S101
    assert "iv=K" in crypto  # noqa: S101
    assert "Base64-String" in crypto  # noqa: S101

    key = b"0123456789abcdef"
    body = {"cmd": 121, "workModel": 1}
    encrypted = encrypt_mqtt_body(body, key)
    ciphertext = base64.b64decode(encrypted, validate=True)
    assert json.loads(_decrypt_aes_cbc(ciphertext, key, key)) == body  # noqa: S101
...
def test_entity_field_candidates_source_truth_shape() -> None:
    ...
    assert by_class["home/SystemBody"] >= {  # noqa: S101
        "soc",
        "batInPw",
        "batOutPw",
        "gridInPw",
    ...
    assert by_class["accessory/AccCTBody"] >= {"volt1", "curr1", "power1", "freq"}  # noqa: S101
    assert by_class["home/UserSystemListApi$Bean"] >= {  # noqa: S101
        "bluetoothKey",
        "deviceSn",
        "systemSn",
        "region",
    }
    assert by_class["DeviceDetailApi$DeviceInfo"] >= {  # noqa: S101
        "deviceSn",
        "modelName",
        "onlineStatus",
    }
