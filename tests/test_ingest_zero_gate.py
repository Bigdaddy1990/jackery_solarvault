...
def test_unconfirmed_zero_drop_is_logged(
    ...
    with caplog.at_level(
        logging.DEBUG,
        logger="custom_components.jackery_solarvault.ingest",
    ):
        gate_payload_section(
            TransportSource.HTTP,
            PAYLOAD_DEVICE_STATISTIC,
...
def test_non_periodic_live_payload_is_not_accepted_by_ingest_gate() -> None:
    """Live payloads must bypass ingest; accidental routing is dropped."""
    result = gate_payload_section(
...
def test_all_zero_series_does_not_confirm_zero_period_payload() -> None:
    """Finite zero samples alone must not turn an unconfirmed gap into data."""
    result = gate_payload_section(
        TransportSource.HTTP,
        APP_SECTION_PV_STAT,
        {APP_CHART_SERIES_Y: [0, "0.0", None], APP_STAT_TOTAL_SOLAR_ENERGY: 0},
    )

    assert result == {}


def test_list_only_all_zero_day_series_is_dropped() -> None:
    """An all-zero chart list is numeric evidence even without scalar totals."""
    result = gate_payload_section(
        TransportSource.HTTP,
        f"{APP_SECTION_PV_STAT}_day",
        {APP_CHART_SERIES_Y: [0, "0.0", None]},
    )
...
_ZERO_CT_WEEK = {
    "totalInCtEnergy": "0.00",
    "totalOutCtEnergy": "0.00",
    "unit": "kWh",
    "y1": [],
    "y2": [],
}
...
_ZERO_EPS_WEEK = {
    "totalInEpsEnergy": "0.00",
    "totalOutEpsEnergy": "0.00",
    "unit": "kWh",
    "y1": [],
    "y2": [],
}
...
def test_ct_sections_bypass_the_ingest_gate_entirely() -> None:
    """CT payloads pass unfiltered on both live and recorder paths.

    Owner directive 2026-07-25: CT values originate in the Shelly cloud
    and are delivered correct — ingest has no business touching them.
    """
    for for_recorder in (True, False):
        result = gate_payload_section(
            TransportSource.HTTP,
            "device_ct_stat_week",
            dict(_ZERO_CT_WEEK),
            for_recorder=for_recorder,
        )

        assert result == _ZERO_CT_WEEK


def test_zero_only_period_payload_reaches_live_state() -> None:
    """Live ingest shows genuine cloud zeros instead of dropping them.

    Regression 2026-07-24: every EPS period sensor sat at "unknown"
    forever because the unconfirmed-zero drop (a Recorder safeguard,
    AGENTS.md §2.2 rule 7) also ran on the live entity path where no
    sibling confirmation exists for EPS sections. Live values fire
    directly; the drop stays recorder-only.
    """
    result = gate_payload_section(
        TransportSource.HTTP,
        "device_eps_stat_week",
        dict(_ZERO_EPS_WEEK),
        for_recorder=False,
    )

    assert result == _ZERO_EPS_WEEK


def test_zero_only_period_payload_still_dropped_for_recorder() -> None:
    """The Recorder path keeps the unconfirmed-zero protection unchanged."""
    result = gate_payload_section(
        TransportSource.HTTP,
        "device_eps_stat_week",
        dict(_ZERO_EPS_WEEK),
...
def test_hierarchy_gate_never_withholds_ct_sections() -> None:
    """CT sections survive the recorder hierarchy gate even when flagged."""
    payload = {
        "device_ct_stat_week": {"totalInCtEnergy": "5.0"},
        "device_pv_stat_week": {"totalSolarEnergy": "8"},
    }

    result = gate_period_hierarchy_for_recorder(
        payload,
        frozenset({"device_ct_stat_week", "device_pv_stat_week"}),
    )

    assert result == {"device_ct_stat_week": {"totalInCtEnergy": "5.0"}}
