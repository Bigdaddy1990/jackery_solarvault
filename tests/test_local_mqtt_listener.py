...
def test_local_mqtt_listener_disabled_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_local_mqtt_listener_enabled_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delattr(sys.modules.get("homeassistant.components", MagicMock()), "mqtt", raising=False)
...
def test_local_mqtt_listener_subscribes_to_expected_topics(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("custom_components.jackery_solarvault.coordinator.ha_mqtt", mock_ha_mqtt):
        import asyncio
        asyncio.run(coordinator.async_start_local_mqtt_listener())
...
def test_local_mqtt_listener_handles_subscribe_failure(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_local_mqtt_listener_message_handler_passes_to_mqtt_handler(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        loop.run_until_complete(coordinator._async_handle_mqtt_message(str(mock_message.topic), payload))
    finally:
        loop.close()
...
def test_local_mqtt_listener_ignores_non_dict_payload(monkeypatch: pytest.MonkeyPatch) -> None:
