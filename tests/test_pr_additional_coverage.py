...
class _FakeEntry:
    ...
    def __init__(
        ...
        # _async_start_local_mqtt gates the client on _entry_owns_coordinator,
        # which checks ``entry.runtime_data is coordinator``. Tests pass this
...
def test_handle_message_bytearray_json_array_increments_dropped() -> None:
    ...
    # A marker inside the array passes the byte prescan; the non-dict JSON
...
@_skip_local_mqtt
def test_handle_message_foreign_json_is_ignored_without_sink_dispatch() -> None:
    """Foreign broker JSON (no Jackery marker) must not parse or reach the sink.

    Regression 2026-07-24: a user-set ``homeassistant`` topic filter on a
    shared broker swept up ~20 foreign messages/s (Zigbee2MQTT, discovery);
    every one was forwarded to the router with ``messages_forwarded`` counting
    broker noise. Foreign payloads are discarded before JSON parsing so Local
    MQTT can never load the event loop the other transports poll on.
    """
    sink_calls: list[str] = []
...
@_skip_local_mqtt
def test_handle_message_jackery_body_only_payload_is_forwarded() -> None:
...
def test_payload_has_jackery_marker_prescan() -> None:
    ...
    assert payload_has_jackery_marker(b'{"messageType": "x"}')
    assert payload_has_jackery_marker('{"deviceSn": "abc"}')
    assert payload_has_jackery_marker(bytearray(b'{"body": {"cmd": 1}}'))
    assert not payload_has_jackery_marker(b'{"battery": 100, "state": "ON"}')
    assert not payload_has_jackery_marker(b"\xff\xfe\xfd")
...
def test_diagnostics_snapshot_started_true_when_runner_task_set() -> None:
    ...
    client = _make_client()
    # is_started is ``task is not None and not task.done()`` — a bare MagicMock
