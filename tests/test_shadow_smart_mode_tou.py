...
async def test_tou_bucket_filled_from_http() -> None:
    ...
    # queryTouPlan is a Home/System config endpoint, so the shadow entry carries
    # the same Home/System context the smart-mode/smart-schedule fills require
    # (a real home-device shadow always includes its system meta).
    working: dict[str, Any] = {
        PAYLOAD_SYSTEM_META: {FIELD_SYSTEM_ID: _SYSTEM_ID},
