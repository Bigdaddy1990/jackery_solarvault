...
from tests.fixtures.jackery_app_2_4_0_contracts import APP_FIELD_EXPOSURE_CONTRACTS  # ruff: ignore[banned-api]  # isort: skip


class TestEntitySourceCapabilities:
    """Test that entity source capabilities match App field contracts."""

    def test_contracts_exist_for_all_app_fields(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
        ...
        for contract in APP_FIELD_EXPOSURE_CONTRACTS:
            ...
            if contract.classification == "entity":
                assert contract.platform, (
                    f"Entity contract missing platform: {contract.model}.{contract.field}"  # ruff: ignore[line-too-long]  # isort: skip
                )
                assert contract.entity_key, (
                    f"Entity contract missing entity_key: {contract.model}.{contract.field}"  # ruff: ignore[line-too-long]  # isort: skip
                )
                assert contract.source_path, (
                    f"Entity contract missing source_path: {contract.model}.{contract.field}"  # ruff: ignore[line-too-long]  # isort: skip
                )
            assert contract.sources, (
                f"Contract missing sources: {contract.model}.{contract.field}"
            )

    def test_entity_sources_subset_of_allowed(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
    ...
    def test_no_entity_claims_transport_it_cannot_use(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
        ...
        for contract in APP_FIELD_EXPOSURE_CONTRACTS:
            if contract.classification == "entity":
                # For entity contracts, verify the source_path exists in coordinator data  # ruff: ignore[line-too-long]  # isort: skip
    ...
    def test_internal_fields_have_rationale(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
        """Internal (non-entity) fields must have documented rationale."""
        for contract in APP_FIELD_EXPOSURE_CONTRACTS:
            if contract.classification == "internal":
                assert contract.rationale, (
                    f"Internal field {contract.model}.{contract.field} missing rationale"  # ruff: ignore[line-too-long]  # isort: skip
                )
    ...
    def test_max_grid_standard_power_in_contracts(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
...
class TestEntitySourceDeclarations:
    ...
    def test_entities_declare_capability_subset(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
...
class TestAppFieldExposureDocumentation:
    ...
    def test_docs_app_field_exposure_exists(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
    ...
    def test_all_contracts_documented(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
        ...
        assert len(entity_contracts) > 20, "Should have many entity contracts"  # ruff: ignore[magic-value-comparison]  # isort: skip
        assert len(internal_contracts) >= 4, "Should have internal field contracts"  # ruff: ignore[magic-value-comparison]  # isort: skip


class TestTranslationSync:
    """Test that translation sync includes new fields."""

    def test_max_grid_standard_power_in_strings(self) -> None:  # ruff: ignore[no-self-use]  # isort: skip
