...
Coordinator command builder all execute unmodified. The BLE-first
...
class _CapturingMqtt:
    ...
    Coordinator command publish only needs ``is_connected`` and
...
def _portable_device_payload() -> dict[str, dict[str, Any]]:
    ...
    succeeds), a legacy-bind-list discovery source (the codebase-wide portable
    gate: ``button._is_portable_payload`` / ``coordinator._is_portable_device_id``
    classify an Explorer/portable unit by ``discovery_source`` on the device
    meta, not by model code), and a properties block carrying no Home/System
    evidence fields so the portable classification is unambiguous.
...
async def portable_setup(
    ...
    real Coordinator command publish path can be observed.
    ...
    coordinator._async_ensure_mqtt = AsyncMock(return_value=None)
...
async def test_button_press_leaves_a_tx_trace(
    ...
    Live finding 2026-07-03: neither Coordinator command publish nor the
