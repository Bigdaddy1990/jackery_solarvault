"""jackery_auth.py — Referenz-Implementierung für eine Jackery Home-Assistant-Integration.
...
def verify_against_capture(
    mqtt_password_b64: str, observed_username: str, observed_password_b64: str
) -> bool:
...
def decrypt_mqtt_password(mqtt_password_b64: str, password_b64: str) -> bytes:
    ...
    iv = key[:16]
    return unpad(
        AES.new(key, AES.MODE_CBC, iv).decrypt(base64.b64decode(password_b64)), 16
    )
...
# ---------------------------------------------------------------------------
def try_decrypt_payload(mqtt_password_b64: str, payload_b64: str) -> bytes | None:
    try:
        key = base64.b64decode(mqtt_password_b64)
        iv = key[:16]
        return unpad(
            AES.new(key, AES.MODE_CBC, iv).decrypt(base64.b64decode(payload_b64)), 16
        )
    except Exception:
        return None
...
# Benötigt zusätzlich: from Crypto.PublicKey import RSA; from Crypto.Cipher import PKCS1_v1_5
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
def payload_encrypt(body_json: str, bluetooth_key_b64: str) -> str:
...
def payload_decrypt(payload_b64: str, bluetooth_key_b64: str) -> str:
...
def build_command_envelope(
    ...
    enc_body = payload_encrypt(
        _json.dumps(body, separators=(",", ":")), bluetooth_key_b64
    )
    env = {
        "deviceSn": device_sn,
        "id": msg_id,
