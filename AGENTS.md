...
The Jackery SolarVault device is controlled and read through three transport
layers with strict priority. **Every contributor and agent MUST follow this
hierarchy.** Violations have caused real production bugs.

| Layer | Transport | Role | Write Access | Read Access |
|-------|-----------|------|:------------:|:-----------:|
| **Layer 3** | HTTP / Cloud API (`fasthttp`) | **PRIMARY** — all commands, all data | **Yes** | **Yes** |
| **Layer 4** | BLE (Bluetooth Low Energy) | **FALLBACK** — commands + live data | Yes | Yes |
| **Layer 5** | MQTT (Cloud + Local + Third-Party) | **DATA SOURCE + COMMAND TRANSPORT** — never primary | Yes (with fallback) | Yes |

#### Rules (non-negotiable)

1. **HTTP/API is always the primary control path.** All set commands (power
   limits, price config, schedules, weather alerts, reboots, etc.) MUST go
   through `self.api.*` HTTP methods first. If HTTP fails, fall back to
   BLE, then MQTT.
2. **BLE is the first fallback.** `_async_publish_command_ble_first` tries
   BLE first, then MQTT. This is correct and MUST be preserved.
3. **MQTT is NEVER the sole coordinator.** MQTT may carry commands
   (encrypted via Layer C / `encrypt_mqtt_body`), but it MUST always have
   an HTTP or BLE fallback path. MQTT alone must never be the only way to
   control the device.
4. **MQTT buttons have DIFFERENT sub-commands** than HTTP/API. The MQTT
   command body format (Layer C AES-128-CBC encrypted) differs from the
   HTTP form-encoded or JSON body. Do NOT merge these code paths.
5. **Data flows INTO HA from all three layers.** Cloud API provides the
   authoritative cache; BLE provides live local values; MQTT provides
   real-time push updates. The coordinator merges these with
   `merge_live_properties`.
6. **Never remove MQTT from button paths.** The user explicitly requires
   MQTT as a command transport with HTTP fallback — not as a replacement
   for HTTP.

#### Command flow for buttons

```
Button Press → coordinator.async_*()
  ├─ _async_publish_command_ble_first()
  │   ├─ TRY: BLE write (cmd via GATT)
  │   └─ FALLBACK: _async_publish_command() → MQTT publish (Layer C encrypted)
  └─ OR: self.api.async_*() → HTTP PUT/POST (primary for setters)
```

#### Data flow for sensors

```
Cloud API (GET /v1/device/*) → coordinator.data  ← authoritative
BLE notifications            → coordinator.data  ← live local override
MQTT push (hb/app/*/device)  → coordinator.data  ← real-time merge
```
...
- **Crypto Layer A:** Der upstream-Key `b"1234567890123456"` wird direkt verwendet
  (`custom_components/jackery_solarvault/const.py:278`). Dies ist eine
