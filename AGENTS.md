...
|--------------------------|----------|---------------|--------|--------|
| HTTP-Endpoints           | 112      | 29            | 5,9 % | partial |
| MQTT-Msg-Types (home)    | 25       | 14            | 5 %   | partial |
| MQTT-Msg-Types (portable)| n/a      | n/a           | n/a    | nicht anwendbar (MQTT = home only) |
| Commands (home)          | 47       | 42            | 8,4 % | partial (5 fehlen) |
| Commands (portable)      | 41       | 0             | 0 %    | intentional (SolarVault home only) |
| Device-Modelle           | runtime  | runtime       | n/a    | ok (`/v1/device/system/list`) |
| Accessories              | 14       | 14            | 0 %  | ok |
| Shelly Cloud2Cloud       | 7        | 3             | 4,9 % | partial |
| Crypto Layer A (auth)    | 1        | 1             | 0 %  | **deviation** (hardcoded `b"1234567890123456"`) |
| Crypto Layer B (signing) | 1        | 1             | 0 %  | ok |
| Crypto Layer C (MQTT)    | 1        | 1             | 0 %  | ok (AES-128-CBC/PKCS7, key=bluetoothKey) |
| Services (HA)            | 7        | 7             | 0 %  | 7/7 in `strings.json` registriert |
| Test-Files               | 45       | 45            | 0 %  | tracked in git |

**Bekannte Abweichungen vom Reference-Protokoll:**

- **Crypto Layer A:** Der upstream-Key `b"1234567890123456"` wird direkt verwendet
  (`custom_components/jackery_solarvault/const.py:157`). Dies ist eine
