...
from dataclasses import dataclass, field as dataclass_field  # noqa: I001
import asyncio
import binascii
from collections.abc import Awaitable, Callable, Iterable, Mapping  # noqa: TC003
import contextlib
from datetime import date, datetime, timedelta, tzinfo  # noqa: TC003
import importlib
import json
import logging
import math
...
from homeassistant.config_entries import ConfigEntry  # noqa: TC001
from homeassistant.core import CoreState, HomeAssistant  # noqa: TC001
from homeassistant.exceptions import ConfigEntryAuthFailed, HomeAssistantError
from homeassistant.helpers.storage import Store
from homeassistant.helpers.update_coordinator import DataUpdateCoordinator, UpdateFailed
from homeassistant.util import dt as dt_util

from .client import JackeryApi, JackeryAuthError, JackeryError  # noqa: TC001
...
from .util import config_entry_bool_option, first_nonblank_int, safe_int
from .const import (
...
def _load_mqtt_push_client() -> type[Any]:
    ...
    return cast(type[Any], module.JackeryMqttPushClient)  # noqa: TC006
...
# --- module constants restored from 24.05 lineage ---  # noqa: E303, RUF100
...
class JackerySolarVaultCoordinator(DataUpdateCoordinator[dict[str, dict[str, Any]]]):  # noqa: E302, PLR0904, RUF100
    ...
    def _is_property_device_candidate(dev: dict[str, Any]) -> bool:
        ...
        if bind_key in (0, "0", False):  # noqa: PLR6201
            return False
    ...
    def _should_skip_fast_property_fetch(self) -> bool:
        ...
        cadence) is the only call we may suppress when a push transport is
        delivering main-device property state at < ``MQTT_LIVE_THRESHOLD_SEC``
        cadence. Generic MQTT traffic is deliberately ignored here.

        Within the ``ADAPTIVE_KEEPALIVE_INTERVAL_SEC`` window the property
        fetch is skipped only while the slow HTTP endpoints are not in active
        backoff. A skipped property endpoint is not evidence that HTTP
        statistics are fresh; diagnostics expose the actual app-chart coverage.
        """
        if not self.data:
            return False
        if any(
            not isinstance(entry, dict) or not entry.get(PAYLOAD_HTTP_PROPERTIES)
            for entry in self.data.values()
        ):
            return False
        now_monotonic = time.monotonic()
        if self._endpoint_backoff_active_count(now_monotonic) > 0:
            return False
        push_elapsed = now_monotonic - self._last_property_push_monotonic
        if push_elapsed > MQTT_LIVE_THRESHOLD_SEC:
            return False
        since_last_refresh = now_monotonic - self._last_http_refresh_completed_monotonic
    ...
    async def async_start_ble_transport(self) -> None:  # noqa: PLR0915
        ...
        if not config_entry_bool_option(
            ...
            return
        try:
            from .client.ble_transport import (  # noqa: TC001
                BleFrameObservation,
                JackeryBleListener,
            )
        ...
        async def _sink(device_id: str, observation: BleFrameObservation) -> None:  # noqa: PLR0911, PLR0912
            ...
            * Other cmds (e.g. ``120`` lifetime stat snapshots) are
            ...
            if cmd in (MQTT_CMD_DEVICE_PROPERTY_CHANGE, MQTT_CMD_CONTROL_COMBINE):  # noqa: PLR6201
    ...
    def _ble_address_for_device(self, device_id: str) -> str | None:
        ...
        if self._ble_listener is None:
            return None
        return cast(str | None, self._ble_listener.address_for_device_id(device_id))  # noqa: TC006
    ...
    async def _async_handle_mqtt_message(  # noqa: C901, PLR0912, PLR0914, PLR0915
        ...
        is_time_zone_config = action_id in (  # noqa: PLR6201
            ACTION_ID_GET_TIME_ZONE,
            ACTION_ID_SEND_TIME_ZONE,
        ) or body.get(FIELD_CMD) in (MQTT_CMD_GET_TIME_ZONE, MQTT_CMD_SEND_TIME_ZONE)  # noqa: PLR6201
        ...
        if topic.endswith("/device") or topic.endswith("/config"):  # noqa: PIE810
            if body:
                if is_wifi_config:
            ...
            updated[PAYLOAD_ALARM] = body if body else payload  # noqa: FURB110
            touched = True
        ...
        if (
            ...
            updated[PAYLOAD_WEATHER_PLAN] = body if body else payload  # noqa: FURB110
            touched = True
        ...
        if (
            ...
            updated[PAYLOAD_TASK_PLAN] = body if body else payload  # noqa: FURB110
            touched = True
        ...
        if (
            ...
            and (
                ...
                or body.get(FIELD_CMD)
                in (MQTT_CMD_QUERY_COMBINE_DATA, MQTT_CMD_CONTROL_COMBINE)  # noqa: PLR6201
            )
            and body
        ):
            property_body = self._strip_lifetime_counters(body)
        ...
        if (
            ...
            source = body if body else payload  # noqa: FURB110
    ...
    def _shelly_cloud_api_device_id(cls, item: dict[str, Any]) -> str | None:
        ...
        if is_shelly:
            ...
            if direct_id not in (None, "") and not str(direct_id).isdecimal():  # noqa: PLR6201
    ...
    def _subdevice_dev_type(cls, item: Mapping[str, Any]) -> int | None:
        ...
        if raw_type not in (None, ""):  # noqa: PLR6201
    ...
    def _subdevice_identity_values(cls, item: Mapping[str, Any]) -> set[str]:
        ...
        for key in (
            ...
            value = item.get(key)
            if value not in (None, ""):  # noqa: PLR6201
    ...
    def _valid_price_sources(sources: object) -> list[dict[str, Any]]:
        ...
        for item in sources:
            ...
            if company_id in (None, "") or not region:  # noqa: PLR6201
                continue
            valid.append(item)
        return valid
    ...
    def _device_country_code(self, device_id: str) -> str | None:
        ...
        for section_name in (PAYLOAD_SYSTEM, PAYLOAD_DEVICE, PAYLOAD_DISCOVERY):
            ...
            if raw not in (None, ""):  # noqa: PLR6201
    ...
    def _find_matching_price_source(
        ...
        if company_id in (None, ""):  # noqa: PLR6201
            return None
        ...
        if not matches:
            return None
        if region not in (None, ""):  # noqa: PLR6201
    ...
    async def async_send_device_schedule(
        ...
        *,
        ...
        """
        if action_id not in (  # noqa: PLR6201
            ACTION_ID_TIMER_TASK_ADD,
            ACTION_ID_TIMER_TASK_DELETE,
            ACTION_ID_TIMER_TASK_UPDATE,
            ACTION_ID_TIMER_TASK_READ,
    ...
    async def async_read_device_schedule(
        ...
        *,
        ...
        if task_type_int not in (  # noqa: PLR6201
            TIMER_TASK_TYPE_SMART_PLUG,
            TIMER_TASK_TYPE_CUSTOM_MODE,
            TIMER_TASK_TYPE_TIME_ELEC,
        ):
    ...
    async def async_set_ct_phase(self, device_id: str, ct_sn: str, phase: int) -> None:
        ...
        phase_int = int(phase)
        if phase_int not in (1, 2, 3, 4):  # noqa: PLR6201
    ...
    def _entity_statistics_from_contributions(
        ...
        *,
        ...
        for start, value, reset_period, cumulative_state in sorted(
            contributions,
            key=lambda item: item[0],  # noqa: FURB118
        ):
    ...
    async def _async_import_app_chart_entity_statistics_for_device(  # noqa: PLR0912, PLR0914, PLR0915
        ...
        *,
        ...
        for statistic_id, entity_contributions in sorted(contributions.items()):
            filtered = [
                item
                for item in sorted(entity_contributions, key=lambda row: row[0])  # noqa: FURB118
            ...
            try:
                async_import_statistics(
                    self.hass,
                    metadata,
                    cast(Iterable[Any], statistics),  # noqa: TC006
                )
    ...
    def statistics_backfill_diagnostics(self) -> dict[str, Any]:
        ...
        for index, device_id in enumerate(
            sorted(devices, key=lambda value: str(value)),  # noqa: PLW0108
            start=1,
        ):
            ...
            for prefix, stat_keys in self._SYSTEM_YEAR_BACKFILL_STAT_KEYS.items():
                ...
                if prefix == APP_SECTION_HOME_TRENDS:
                    ...
                    for month, source in zip(previous_months, sources, strict=False):
                        if isinstance(source, dict):
                            months[month] = source  # noqa: PERF403
                if months:
                ...
                if sys_id:
                    ...
                    tasks.append(
                        _get_with_ttl_for(
                            ...
                            self._slow_metrics_interval_sec,
                            cast(
                                Callable[[], Awaitable[dict[str, Any]]],  # noqa: TC006
    ...
    def _is_battery_pack_lifetime_ble_payload(body: dict[str, Any]) -> bool:
        ...
        return dev_type in (None, SUBDEVICE_DEV_TYPE_BATTERY_PACK)  # noqa: PLR6201
    ...
    def _encode_third_party_mqtt_secrets(
        ...
        if not any(body.get(field) for field in secret_fields):
            return body
        from .client.ble import BLE_AES_IV_LEN  # noqa: PLC0415, RUF100
        ...
        if key is None or len(key) != BLE_AES_IV_LEN:
            ...
            return body
        from .client.third_party_mqtt_codec import encode_third_party_mqtt_field  # noqa: I001, PLC0415, RUF100
    ...
    async def async_device_get_ota_page_data(
        ...
        """Request a specific OTA firmware page from the device (actionId 3009, cmd 103)."""  # noqa: E501, RUF100
        await self._async_publish_command(
            device_id,
            message_type=MQTT_MESSAGE_DEVICE_PROPERTY_CHANGE,
...
def _exception_debug_message(err: BaseException) -> str:  # noqa: E302, RUF100
    """Return a useful debug message for exceptions with empty ``str(err)``."""
    return f"{type(err).__name__}: {err or '(no message)'}"


def _control_int(value: Any, field_name: str) -> int:  # noqa: ANN401, E302, RUF100
...
def _transport_cmd(value: Any) -> int:  # noqa: ANN401, E302, RUF100
