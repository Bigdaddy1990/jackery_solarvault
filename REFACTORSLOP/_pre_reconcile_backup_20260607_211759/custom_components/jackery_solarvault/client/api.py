...
from ..util import safe_bool, app_period_date_bounds, chart_series_debug
...
class JackeryError(Exception):  # noqa: E302, RUF100
    """Base exception."""
...
class JackeryApi:  # noqa: PLR0904
    ...
    def _response_has_auth_failure_text(data: object) -> bool:
        ...
        text = " ".join(str(part) for part in parts if part not in (None, "")).lower()  # noqa: PLR6201
        if not text:
            return False
        return any(
            marker in text
    ...
    def _is_auth_failure_response(self, status: int, data: object) -> bool:
        ...
        if status in (401, 403):  # noqa: PLR6201
            return True
        ...
        code = self._extract_code(data)
        return code not in (CODE_OK, None) and self._response_has_auth_failure_text(  # noqa: PLR6201
            data
        )
    ...
    async def _get_json(self, path: str, params: dict | None = None) -> dict:
        ...
        code = self._extract_code(data)
        if code not in (CODE_OK, None):  # noqa: PLR6201
            raise JackeryApiError(  # noqa: TRY003
...
def _write_accepted(data: dict[str, Any]) -> bool:  # noqa: E302, RUF100
