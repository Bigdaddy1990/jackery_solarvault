...
from homeassistant.config_entries import ConfigEntry  # noqa: I001, TC001
import json
import logging
import math
...
SEND_DEVICE_SCHEDULE_SCHEMA = vol.Schema({
    ...
    vol.Required(SERVICE_FIELD_ACTION_ID): vol.All(
        vol.Coerce(int), vol.In(MQTT_ACTION_IDS_SCHEDULE)
    ),
...
def _text_field(  # noqa: E302, PLR0913, RUF100
    call: ServiceCall,
    field: str,
    *,
    translation_key: str,
...
def _device_id_field(call: ServiceCall, translation_key: str) -> str:  # noqa: E302, RUF100
...
def _optional_text(call: ServiceCall, field: str, label: str) -> str:  # noqa: E302, RUF100
    raw = call.data.get(field, "")
    if raw is None:
        return ""
    if not isinstance(raw, str):
