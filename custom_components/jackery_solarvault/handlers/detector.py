...
def subdevice_dev_type(item: Mapping[str, Any]) -> int | None:
    ...
    if raw_type not in {None, ""}:
        try:
            ...
            _LOGGER.debug(
                "subdevice_dev_type: non-numeric devType %r, falling back to "
                "scan name: %s",
                raw_type,
                err,
            )
...
def has_sub_device_accessory(payload: dict[str, Any]) -> bool:
    ...
    # This bucket (sub_devices) can contain multiple types (Smoke, Leak, etc.)
    # We check if any exist.
...
def battery_packs_need_query(payload: dict[str, Any]) -> bool:
    ...
    try:
        ...
        _LOGGER.debug(
            "battery_packs_need_query: non-numeric %s %r, assuming 0: %s",
            FIELD_BAT_NUM,
