...
def _normalize_account(value: Any) -> str:  # noqa: ANN401
    """Normalize user-facing account identifiers before authentication and unique-id.
    ...
    Returns:
        str: The trimmed account string if `value` is a `str`, otherwise an empty
        string.
    """
...
def _entry_text(entry: ConfigEntry, key: str) -> str:
    ...
    Returns:
        str: The stored string value for `key`, or an empty string if the key is
...
def _current_option_values(entry: ConfigEntry) -> dict[str, bool]:
    """Retrieve current option values for the given config entry, using legacy defaults.

    when an option is not set.

    Parameters:
        entry (ConfigEntry): The configuration entry to read options from.

    Returns:
        dict[str, bool]: Mapping of option keys to their boolean values; stored option
        values are used when present, otherwise legacy defaults are applied.
    """
    return {
...
def _flow_options(
    ...
    For each option key defined in _OPTION_DEFAULTS, prefer the value from user_input
    when present, otherwise use current_options when provided, otherwise fall back to
    the configured default.

    Parameters:
        user_input (dict[str, Any]): Form-submitted option values keyed by option name.
        current_options (dict[str, bool] | None): Existing option values to preserve
        when a key is omitted.

    Returns:
        dict[str, bool]: Mapping of every known option key to its resolved boolean
        value (`True` or `False`).
    """
...
def _entry_data_from_api_login(
    ...
    Stores CONF_USERNAME and CONF_PASSWORD. Adds CONF_MQTT_MAC_ID and CONF_REGION_CODE
    from the API when available; if not available, uses those keys from existing_entry
    only when they exist as strings. If the API provides an MQTT session snapshot,
    includes a copy under ENTRY_BOOTSTRAP_MQTT_SESSION.

    Parameters:
        account (str): Username to store under CONF_USERNAME.
        password (str): Password to store under CONF_PASSWORD.
        api (JackeryApi): Authenticated API client exposing `mqtt_mac_id`,
        `region_code`, and `mqtt_session_snapshot()`.
        existing_entry (ConfigEntry | None): Optional existing entry whose string
        fields are used as fallbacks.

    Returns:
        dict[str, Any]: Mapping containing at minimum CONF_USERNAME and CONF_PASSWORD,
        and optionally CONF_MQTT_MAC_ID, CONF_REGION_CODE, and
        ENTRY_BOOTSTRAP_MQTT_SESSION.
    """
    data: dict[str, Any] = {
    ...
    if mqtt_mac_id:
        ...
        existing_entry.data.get(CONF_MQTT_MAC_ID),
        str,
    ):
...
def _reconfigure_options(
    ...
    Preserves any option keys not exposed by the reconfigure form, applies submitted
    boolean option toggles from `user_input`, and ensures local-MQTT-related fields are
    taken from the existing entry rather than the form.
...
class JackeryOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        If form input is provided, merge the submitted boolean option toggles and local
        MQTT fields with the current entry values and create the options entry. If no
        input is provided, present the options form populated with the current boolean
        option defaults and current local MQTT settings.

        Parameters:
            user_input (dict[str, Any] | None): Submitted form values for option
            toggles and local MQTT fields; may be None when rendering the form.

        Returns:
            ConfigFlowResult: A config flow result that either creates the updated
            options entry or displays the options form.
        """
...
class JackeryConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def _async_abort_duplicate_discovery(self) -> ConfigFlowResult | None:
        """Abort discovery flows when the integration is already configured or another.

        flow is in progress.

        If an existing configured entry is present, the flow is aborted with reason
        "already_configured". If another flow for this integration is in progress
        (excluding the current flow), the flow is aborted with reason
        "already_in_progress".

        Returns:
            ConfigFlowResult | None: An abort result with the appropriate reason when a
    ...
    async def async_step_reconfigure(
        ...
        """Reconfigure an existing config entry by validating the provided credentials.

        and applying updated entry data and options.

        Validates that the normalized username in `user_input` matches the entry being
        ...
        Returns:
            ConfigFlowResult: A result that either shows the reconfigure form with
        ...
        if user_input is not None:
            ...
            if not account:
                ...
                try:
                    ...
                    # Contract compatibility: data_updates={CONF_USERNAME: account,
    ...
    async def async_step_reauth_confirm(
        ...
        """Prompt for the account's current password, validate it with the Jackery API,.

        and update the config entry on success.

        Aborts if the reauthentication target entry or its stored username cannot be
        retrieved. On successful authentication, updates the entry data with
        API-derived bootstrap fields, persists any MQTT session snapshot, and aborts
        with FLOW_ABORT_REAUTH_SUCCESSFUL. On authentication or connection failure,
        re-displays the password form with an appropriate error.

        Returns:
            ConfigFlowResult: a form to collect a password, an abort result, or an
