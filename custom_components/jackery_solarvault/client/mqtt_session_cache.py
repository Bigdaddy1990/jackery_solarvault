...
async def async_load_mqtt_session(  # noqa: PLR0911
    ...
    """Load cached MQTT session credentials for the given config entry from persistent
    storage.
...
async def async_save_mqtt_session(  # noqa: PLR0913
    ...
    *,
    ...
    async def _async_save_transaction() -> None:
        """Merge this entry's MQTT session under the shared storage lock."""
        async with transaction_lock:
    ...
    task = hass.async_create_task(
        ...
        name=f"{DOMAIN}_mqtt_session_cache_save_{entry_id}",
        eager_start=False,
    )
...
async def async_clear_mqtt_session(hass: HomeAssistant, entry_id: str) -> None:
    ...
    async def _async_clear_transaction() -> None:
        """Remove this entry's MQTT session under the shared storage lock."""
        async with transaction_lock:
            loaded = await store.async_load()
    ...
    task = hass.async_create_task(
        _async_clear_transaction(),
        name=f"{DOMAIN}_mqtt_session_cache_clear_{entry_id}",
        eager_start=False,
    )
    await asyncio.shield(task)
