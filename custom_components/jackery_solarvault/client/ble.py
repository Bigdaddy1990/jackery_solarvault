...
#: Largest unsigned value carried by the protocol's fixed-width fields.
_UINT16_MAX: int = 0xFFFF

#: Key lengths (in bytes) accepted by the BLE crypto helpers.
#:
#: PROTOCOL.md §14 originally documented a fixed 32-byte AES-256 key, but a
...
# The independent wire fields remain public keyword arguments for compatibility.
def build_binary_frame(  # ruff: ignore[too-many-arguments]
    *,
    ...
    if trailer is not None and len(trailer) != _BINARY_FRAME_TRAILER_LEN:
        ...
        raise ValueError(msg)
    if security is not None and not 1 <= security <= _UINT16_MAX:
        msg = f"security {security} out of range"
        raise ValueError(msg)
    if trailer is not None and security is not None:
        msg = "trailer and security are mutually exclusive"
        raise ValueError(msg)
    header = (
    ...
    actual_security = (
        secrets.randbelow(_UINT16_MAX) + 1 if security is None else security
    )
