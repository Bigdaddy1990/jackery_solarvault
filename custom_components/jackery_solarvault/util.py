...
def _trend_bucket_energy_value(
    raw: object,
    section: str,
    stat_key: str,
) -> float | None:
...
def _is_signed_output_energy_stat(section: str, stat_key: str) -> bool:
    """Return true for app chart directions that may arrive as negative values."""
    return (
        (
        ...
        or (
            ...
            and stat_key == APP_STAT_TOTAL_OUT_GRID_ENERGY
        )
        or (
            section.startswith(APP_SECTION_CT_STAT)
            and stat_key == APP_STAT_TOTAL_CT_OUTPUT_ENERGY
        )
        or (
            section.startswith(APP_SECTION_EPS_STAT)
            and stat_key == APP_STAT_TOTAL_OUT_EPS_ENERGY
        )
    )
