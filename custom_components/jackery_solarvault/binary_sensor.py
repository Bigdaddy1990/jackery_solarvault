...
# accessory (and any unmapped devType) falls back to the description's
# `safety` device_class.
SUBDEVICE_ALARM_DEVICE_CLASSES: dict[int, BinarySensorDeviceClass] = {
...
async def async_setup_entry(  # ruff:ignore[unused-async]  # HA awaits this entry point
    ...
    def _append_unique(
        ...
        """Add the entity to the provided list and record its unique ID if that ID has
    ...
    def _collect_entities() -> list[BinarySensorEntity]:
        ...
        For each device, create one JackeryBinarySensor per description in
        BINARY_DESCRIPTIONS and one JackerySmartPlugStateBinarySensor for each smart
        plug that has a serial number. Smart-plug entities capture a 1-based plug index
        and the plug serial to maintain stable binding across payload reorders.

        Returns:
            list[BinarySensorEntity]: Constructed binary sensor entities ready to be
            added.
        """
        entities: list[BinarySensorEntity] = []
...
class JackerySmartPlugStateBinarySensor(JackeryEntity, BinarySensorEntity):
    ...
    def _plug(self) -> dict[str, Any]:
        ...
        """Finds the smart-plug payload matching this entity's captured serial to keep
        the entity bound to the same physical plug if the cloud-side plug list is
        reordered.
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return diagnostic attributes for the smart plug, always including its 1-based
        `plug_index`.

        Includes any of the following fields from the matched plug payload when present:
...
class JackerySubdeviceAlarmBinarySensor(JackeryEntity, BinarySensorEntity):
    """Alarm state for a subdevice."""
    ...
    def device_class(self) -> BinarySensorDeviceClass | None:
        """Return the device_class matching this subdevice's accessory type.
        ...
        return SUBDEVICE_ALARM_DEVICE_CLASSES.get(
            dev_type,
