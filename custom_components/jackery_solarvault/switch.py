...
from .handlers.exceptions import ACTION_WRITE_ERRORS
from .util import (
...
class JackeryDescriptionSwitch(JackeryEntity, SwitchEntity):
    ...
    def is_on(self) -> bool | None:
        ...
        `source_keys` value, the optional `fallback_section`, and the task-plan
        fallback when `use_task_plan_fallback` is enabled. If no value is found the
        state is unknown.
...
class JackeryBreakerSwitch(JackeryEntity, SwitchEntity):
    ...
    _attr_icon = "mdi:circuit-breaker"
...
class JackerySmartPlugPrioritySwitch(JackerySmartPlugSwitch):
    ...
    async def _async_set_state(self, value: bool) -> None:
        """Set the smart plug's priority enabled state and request a coordinator.
