...
from .client.mqtt.local_mqtt import (
...
# older typing APIs.
if not TYPE_CHECKING:  # ruff:ignore[non-empty-init-module]
...
def _async_clean_legacy_entities(  # ruff:ignore[non-empty-init-module]
    ...
    """Remove entity-registry entries that are obsolete or disabled by the entry's.

    options.

    This scans and removes stale energy helper entities and legacy sensors/binary
    sensors whose unique IDs match known legacy suffixes. Sensors associated with
    optional features are removed when the corresponding config entry option is
    disabled. The function performs in-place removals in Home Assistant's entity
    registry.
    """
...
_STARTUP_TASK_RUNTIME_KEY = "startup_task"  # ruff:ignore[non-empty-init-module]
...
def _entry_startup_task(  # ruff:ignore[non-empty-init-module]
    hass: HomeAssistant,
    entry: ConfigEntry,
    ...
    Returns:
        The asyncio.Task instance for the entry's startup task, or `None` if no task is
        registered or the runtime bucket is missing/invalid.
    """
...
async def _async_cancel_startup_task(hass: HomeAssistant, entry: ConfigEntry) -> None:  # ruff:ignore[non-empty-init-module]
...
async def _async_call_if_present(obj: object, name: str) -> None:  # ruff:ignore[non-empty-init-module]
    ...
    Parameters:
        ...
        name (str): The attribute name to call. The attribute will be invoked with no
        arguments;
...
async def _async_prime_entry_bootstrap_mqtt_session(  # ruff:ignore[non-empty-init-module]
    hass: HomeAssistant,
    entry: ConfigEntry,
    api: JackeryApi,
) -> dict[str, str] | None:
...
async def _async_authenticate_api_layer(  # ruff:ignore[non-empty-init-module]
    ...
    """Authenticate the cloud API layer and persist an updated MQTT session snapshot.

    when appropriate.

    Primes the API with any bootstrap or cached MQTT session, performs an HTTP login,
    and, after a successful login,
    saves the API's current MQTT session snapshot to persistent storage if it differs
    from the cached or bootstrap snapshot.
...
def _defer_coordinator_auth_failure(  # ruff:ignore[non-empty-init-module]
    coordinator: JackerySolarVaultCoordinator,
    err: ConfigEntryAuthFailed,
) -> None:
    """Record an authentication failure on the coordinator for later background.
    ...
    """
    coordinator._defer_background_auth_failure(err)  # ruff:ignore[private-member-access]
    coordinator._mqtt_auth_failure_message = str(err)  # ruff:ignore[private-member-access]


async def _async_discover_with_cache_fallback(  # ruff:ignore[non-empty-init-module]
    coordinator: JackerySolarVaultCoordinator,
) -> bool:
    ...
    If discovery raises a ConfigEntryAuthFailed, the authentication failure is deferred
    and startup should stop. If discovery raises an UpdateFailed and a cached discovery
    snapshot exists, that snapshot is injected so transports that depend on discovery
    can still start.

    Returns:
        bool: `True` if discovery succeeded or startup may continue; `False` if
        authentication failed and startup must stop.
    """
    try:
        ...
        if cached_snapshot:
            _LOGGER.warning(
                ...
                "entities keep their last state, but Layer 5 startup is blocked "
                "until HTTP discovery succeeds: %s",
                err,
            )
...
def _handle_refresh_startup_result(  # ruff:ignore[non-empty-init-module]
    coordinator: JackerySolarVaultCoordinator,
    result: BaseException | object,
) -> bool:
    """Process the coordinator's initial HTTP refresh result performed during startup.

    If `result` is a `ConfigEntryAuthFailed`, the coordinator's auth failure is
    deferred. If `result` is a `UpdateFailed` and the coordinator has a cached
    discovery snapshot, that cached payload is applied so transports can proceed. If
    `result` is any other exception, a warning is logged. If `result` is not an
    exception, no action is taken.

    Parameters:
        coordinator: The coordinator instance whose state may be updated or where auth
        failures are deferred.
        result: The value or exception produced by the coordinator's first refresh
        attempt.
    """
    ...
    if isinstance(result, UpdateFailed) and (
        ...
        _LOGGER.warning(
            ...
            "entities keep their last state, but Layer 5 startup is blocked until "
            "HTTP refresh succeeds: %s",
            result,
        )
...
def _handle_optional_startup_result(  # ruff:ignore[non-empty-init-module]
    ...
    *,
    ...
    """Process a non-critical startup-task result, deferring auth failures or logging.
...
async def _async_finish_entry_startup(  # ruff:ignore[non-empty-init-module]
    hass: HomeAssistant,
    entry: JackeryConfigEntry,
    coordinator: JackerySolarVaultCoordinator,
) -> None:
    ...
    try:
        ...
        refresh_result = refresh_results[0] if refresh_results else None
...
def _local_mqtt_client(  # ruff:ignore[non-empty-init-module]
    ...
    """Get the per-entry local MQTT client stored in hass.data for the given config.

    entry, if present.

    Returns:
        The JackeryLocalMqttClient for the entry, or `None` if no client is stored or
...
async def _async_start_local_mqtt(  # ruff:ignore[non-empty-init-module]
    hass: HomeAssistant,
    entry: JackeryConfigEntry,
    coordinator: JackerySolarVaultCoordinator,
) -> None:
    ...
    async def _sink(
        ...
        Parameters:
            ...
            data (dict[str, Any] | None): Parsed JSON payload to route; if `None`, the
            message is ignored.
        """
        if data is None:
            return
    ...
    async def _async_stop_local_mqtt() -> None:
        ...
        If stopping the client raises any exception, the exception is suppressed and
        the runtime reference removal is attempted regardless.
        """
        try:
            await client.async_stop()
        except Exception as err:  # ruff:ignore[blind-except]
            _LOGGER.debug("Jackery local MQTT client stop failed on unload: %s", err)
...
async def async_setup_entry(hass: HomeAssistant, entry: JackeryConfigEntry) -> bool:  # ruff:ignore[non-empty-init-module]
    ...
    coordinator and forward platforms first. The background startup task then
    authenticates and refreshes Layer 3 (HTTP) before Layer 5 transports
    (cloud MQTT, local MQTT and BLE) are allowed to start. Background auth failures
    are deferred to the coordinator so a rejected session cannot bootloop Home
    Assistant.
    """
    ...
    try:
        ...
        except Exception as shutdown_err:  # ruff:ignore[blind-except]
            _LOGGER.debug(
                "Jackery coordinator shutdown failed during setup rollback: %s",
                shutdown_err,
            )
    ...
    # Defense-in-depth: tie the startup task's cancellation to the entry unload
    # lifecycle so a pending login (a real ``getaddrinfo``) cannot outlive the
    # entry on teardown paths that bypass ``async_unload_entry``. The explicit
    # ``_async_cancel_startup_task`` in unload still awaits it; this guarantees
    # the cancel even when only ``async_on_unload`` callbacks run.
...
def _legacy_suffix_matches(uid: str, key_suffix: str) -> bool:  # ruff:ignore[non-empty-init-module]
    ...
    Returns:
        bool: `True` if `uid` equals a legacy head concatenated with `key_suffix`,
        `False` otherwise.
    """
    if not key_suffix:
...
def _async_remove_entities_with_suffixes(  # ruff:ignore[non-empty-init-module]
    ...
    *,
    ...
    """Remove entity-registry entries for the given config entry and domain whose.

    legacy unique IDs end with any of the provided legacy suffixes.
...
async def async_unload_entry(hass: HomeAssistant, entry: JackeryConfigEntry) -> bool:  # ruff:ignore[non-empty-init-module]
