...
def _install_unix_only_stubs() -> None:
    """Place no-op stubs for Unix-only stdlib modules HA imports unconditionally.

    The shimmed modules are loaded by ``homeassistant.runner`` /
    ``homeassistant.util.resource`` very early during pytest plugin collection.
    On Windows those imports fail and the HA-fixture pytest plugin never loads,
    so the HA test suite cannot run locally. The stubs return ``0`` / ``None``
    so HA's startup paths that only check for an exception (rather than the
    real syscall result) keep working — the integration's unit tests do not
    exercise real Unix-only file locking or rlimit behaviour.
    """
    if sys.platform != "win32":
        return
