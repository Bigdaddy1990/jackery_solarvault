...
def _bump_release(release, bump_type):  # noqa: ANN001, ANN202
...
def bump_version(  # noqa: PLR0912, PLR0915
    ...
    if bump_type == "minor":
        ...
        if not version.is_devrelease:
            raise ValueError("Can only be run on dev release")  # noqa: TRY003

        new_dev = dt_util.utcnow().strftime("%Y%m%d%H%M")
        if nightly_version:
            new_version = Version(nightly_version)
            if new_version.release != version.release:
                raise ValueError("Nightly version must have the same release version")  # noqa: TRY003
            if not new_version.is_devrelease:
                raise ValueError("Nightly version must be a dev version")  # noqa: TRY003
        ...
        raise ValueError(f"Unsupported type: {bump_type}")  # noqa: TRY003
...
def write_version(version) -> None:  # noqa: ANN001, ANN201
    ...
    Path("homeassistant/const.py").write_text(content, encoding="utf-8")
...
def write_ci_workflow(version: Version) -> None:
    ...
    content = Path(".github/workflows/ci.yaml").read_text(encoding="utf-8")
    ...
    Path(".github/workflows/ci.yaml").write_text(content, encoding="utf-8")
...
def main() -> None:
    ...
    if (
        arguments.commit
        and subprocess.run(["git", "diff", "--quiet"], check=False).returncode == 1  # noqa: S607
    ):
        return
    ...
    assert bumped > current, "BUG! New version is not newer than old version"  # noqa: S101
    ...
    subprocess.run(["git", "commit", "-nam", f"Bump version to {bumped}"], check=True)  # noqa: S603, S607
...
def test_bump_version() -> None:
    ...
    assert bump_version(Version("0.56.0"), "beta") == Version("0.56.1b0")  # noqa: S101
    assert bump_version(Version("0.56.0b3"), "beta") == Version("0.56.0b4")  # noqa: S101
    assert bump_version(Version("0.56.0.dev0"), "beta") == Version("0.56.0b0")  # noqa: S101

    assert bump_version(Version("0.56.3"), "dev") == Version("0.57.0.dev0")  # noqa: S101
    assert bump_version(Version("0.56.0b3"), "dev") == Version("0.57.0.dev0")  # noqa: S101
    assert bump_version(Version("0.56.0.dev0"), "dev") == Version("0.56.0.dev1")  # noqa: S101

    assert bump_version(Version("0.56.3"), "patch") == Version("0.56.4")  # noqa: S101
    assert bump_version(Version("0.56.3.b3"), "patch") == Version("0.56.3")  # noqa: S101
    assert bump_version(Version("0.56.0.dev0"), "patch") == Version("0.56.0")  # noqa: S101

    assert bump_version(Version("0.56.0"), "minor") == Version("0.57.0")  # noqa: S101
    assert bump_version(Version("0.56.3"), "minor") == Version("0.57.0")  # noqa: S101
    assert bump_version(Version("0.56.0.b3"), "minor") == Version("0.56.0")  # noqa: S101
    assert bump_version(Version("0.56.3.b3"), "minor") == Version("0.57.0")  # noqa: S101
    assert bump_version(Version("0.56.0.dev0"), "minor") == Version("0.56.0")  # noqa: S101
    assert bump_version(Version("0.56.2.dev0"), "minor") == Version("0.57.0")  # noqa: S101

    now = dt_util.utcnow().strftime("%Y%m%d%H%M")
    assert bump_version(Version("0.56.0.dev0"), "nightly") == Version(  # noqa: S101
        f"0.56.0.dev{now}"
    )
    assert bump_version(  # noqa: S101
