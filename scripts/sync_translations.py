"""Sync jackery_solarvault translation files with strings.json.
...
def _sync_translation(
    ...
    *,
    ...
    if language_file.exists():
        ...
        existing_data = json.loads(original_normalized)
    else:
        if check_only:
        ...
        language_file.write_text(_dump_json(strings_data), encoding="utf-8", newline="\n")
        return True

    synced = _sync_tree(strings_data, existing_data)
    new_content = _dump_json(synced)

    # Normalize original for comparison (json.dumps always uses \n)
    if new_content != original_normalized:
        if check_only:
            raise SystemExit(f"Translation file out of date: {language_file}")
        language_file.write_text(new_content, encoding="utf-8", newline="\n")
        return True
...
def main(argv: list[str] | None = None) -> int:  # noqa: D103
    ...
    parser.add_argument(
        "--integration-path",
        type=Path,
        default=ROOT / "custom_components" / "jackery_solarvault",
        help="Path to the jackery_solarvault integration directory.",
    )
    parser.add_argument(
        "--languages",
        nargs="*",
